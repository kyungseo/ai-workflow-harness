...
- [ai-workflow-harness](#ai-workflow-harness)
  ...
  - [2. Document Layers](#2-document-layers)
    - [User-Facing References](#user-facing-references)
    - [Load Rule](#load-rule)
  ...
  - [6. STATUS And Work File Rules](#6-status-and-work-file-rules)
    - [STATUS.md](#statusmd)
    - [Work File Lifecycle](#work-file-lifecycle)
  ...
  - [10. New Project Adoption](#10-new-project-adoption)
    - [이 Repository에서 작업](#이-repository에서-작업)
...
- **적용 대상 프로젝트** 구축: AI Workflow의 적용과 운영을 테스트하기 위한 부가 프로젝트 (v1.0을 찍으면서 정리함. 현재 archive 일부와 git log에만 흔적이 남아 있다)

이 Workflow는 첫 commit 이후 한 번에 설계된 단순한 산출물이 아니다. 첫 commit 이후 현재까지 repository에는 **340여 개의 commit**이 쌓였다.
...
> **Note** Approval Matrix는 lifecycle 단계가 아니라, PLAN → APPROVAL → EXECUTE, state-change, commit gate 전반에 걸쳐 적용되는 cross-cutting control rule이다.
...
기본 profile은 framework를 가정하지 않는 `generic`이며, Spring Boot example pack이 필요할 때만 `--profile spring-boot`를 사용한다.

```bash
# 신규 프로젝트: 이 repository의 temp/my-app 아래에 생성
scripts/create-harness.sh my-app
scripts/create-harness.sh --dry-run my-app

# 신규 프로젝트: 원하는 위치에 생성
scripts/create-harness.sh my-app /path/to/my-app
scripts/create-harness.sh --dry-run my-app /path/to/my-app
...
`target-dir`를 생략한 신규 생성은 `scripts/create-harness.sh`가 있는 repository root 기준 `temp/<project-name>/`에 생성된다.
예를 들어 이 repository에서 `scripts/create-harness.sh my-app`을 실행하면 `temp/my-app/`이 만들어진다.
...
[docs/decisions/DR-007-language-policy.md](docs/decisions/DR-007-language-policy.md)

(대부분의 문서는 한글이지만 Agent Core 문서들은 전문을 영어로 작성한다.)
