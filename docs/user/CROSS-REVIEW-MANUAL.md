...
reviewer에게 전달할 때 (reviewer agent 쪽 프롬프트):

```text
아래 relay packet을 받아서 reviewer 역할로 응답해줘.
[패킷 붙여넣기]
```

relay packet 자체에 역할·posture·출력 형식이 포함되어 있으므로 추가 설명 없이 이것으로 충분하다.
reviewer가 참조할 파일이나 코드가 패킷에 포함되지 않았다면 함께 붙여넣는다.
