...
| `ADOPTER-UPGRADE-AGENT-FIRST.md` | adopter upgrade의 **default entry** — route 판정식(Entry Conditions + fallback override)의 SSoT + non-negotiable gate 4 + 최소 체크리스트 |
| `ADOPTER-UPGRADE-MIGRATION-PLAYBOOK.md` | adopter upgrade **fallback playbook**(AGENT-FIRST 판정식이 fallback을 가리키는 모든 case) — temp rehearsal → real apply 절차 + 양 route 공용 authoritative section(Phase 4·6) |
| `PRODUCT-STARTER-PLANNING-PACK.md` | source-first planning pack, source↔product import loop, `base-msa-template` 분석 include/exclude 기준 |
