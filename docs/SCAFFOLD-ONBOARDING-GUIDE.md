...
> **형식 참고**: 아래 `P1-NNN` 식별자는 legacy 형식 예시입니다. 실제 backlog 후보는 착수 전 Work ID 없이 제목/slug로 관리하고, Work ID는 `/work` 착수 승인 시 확정합니다.
...
복잡하거나 여러 단계인 작업 → docs/works/phase1/FEAT-YYYYMMDD-NNN-topic.md 생성 권장
                           착수 시: /work [title-or-slug]
```
...
/work [title-or-slug]    # 사용자가 착수할 작업 선택 (Work ID는 착수 승인 시 확정)
