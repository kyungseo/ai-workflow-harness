...
3. [첫 /start에서 보게 될 내용](#3-첫-start에서-보게-될-내용)
...
12. [§4 Phase 1 Backlog Derivation](#12-4-phase-1-backlog-derivation)
...
- 다음 세션에서 `/start`를 실행하면 bootstrap 반복 없이 실제 작업 후보로 이어지는 상태
...
| §2 Product Definition | 제품 목표와 성공 기준 확정 | Phase 1 목표, 첫 사용자 시나리오, 성공 기준 | `PLAN-SUMMARY.md` Project Summary와 `PLAN.md` 목표 반영 |
...
| §4 Phase 1 Backlog Derivation | Product track 후보 생성 | 요구사항, 우선순위, 착수 후보 | `PHASE1.md` 후보와 Done Criteria/Verification 작성 |
...
따라서 첫 `/start`는 clean idle로 끝나지 않고, `docs/BOOTSTRAP.md`를 후속으로 읽어야 한다고 안내하는 것이 정상이다.
...
4. docs/backlog/PHASE1.md 초기 후보 도출
...
| 도구 | 시작 방법 | 첫 메시지 |
|---|---|---|
| Claude Code | project root에서 `claude` 실행 후 `/start` | `/start` |
| Codex 앱 | scaffold된 project root를 workspace로 열기 | `/start` intent: `AGENTS.md 기준으로 STATUS를 요약하고 bootstrap onboarding 흐름을 제안해줘.` |
| Codex CLI | project root에서 `codex` 실행 | `/start` 또는 같은 의미의 자연어 요청 |
...
**목적:** 사용자가 Phase 1의 목표와 성공 기준을 확정하면, AI가 `docs/PLAN-SUMMARY.md`와 `docs/PLAN.md`에 반영한다.
...
| 항목 | 설명 |
|---|---|
| Phase 1 목표 | 한 문장 — 이번 Phase가 끝났을 때 무엇이 완성되는가 |
...
```
You: Phase 1 목표는 나중에 채울게. 일단 §3으로 넘어가자.

AI : 알겠습니다. §2 placeholder를 유지하고 §3 Project Initialization으로 이동합니다.
     기능 후보 도출은 목표 확정 후 /register로 추가할 수 있습니다.
```
...
## 12. §4 Phase 1 Backlog Derivation

**목적:** 사용자가 요건을 제공하면 AI가 작업 후보를 도출해 `docs/backlog/PHASE1.md`에 등록한다.
**주체:** 요건 제공과 후보 우선순위 결정은 사용자. 후보 작성(Done Criteria / Verification / Preconditions)은 AI.

### AI가 생성하는 항목 형식

```markdown
**P1-001** | Priority: P0 | Scope: 멀티 모듈 프로젝트 초기 구조 세팅
...
```
[P0] P1-001 멀티 모듈 구조 세팅          ← 모든 작업의 시작점
      └── [P0] P1-002 H2 DB + DDL 설정
               ├── [P1] P1-003 common-core 공통 모듈
               │         ├── [P1] P1-004 Security + JWT + Redis
               │         │         ├── [P1] P1-005 Gateway
               │         │         ├── [P1] P1-006 auth 서비스
               │         │         ├── [P1] P1-007 user 서비스
               │         │         └── [P1] P1-008 todo 서비스
               │         └── [P2] P1-012 env 기반 Config
               ├── [P2] P1-009 Swagger (인증/인가 + API 그룹)
               ├── [P2] P1-010 서비스 간 연계 ← OQ-001 결정 후 착수
               ├── [P2] P1-011 MDC 로깅
               └── [P2] P1-013 API 테스트 파일

[P3] P1-014 프론트엔드 (가이드 샘플)
[P3] P1-015 infra / scripts 구조
```

### 시나리오 A — §2 목표가 있는 경우

사용자가 상세 요건을 제공하면 AI가 P1-001부터 기능 후보까지 전체를 도출한다.

```
You: [§2에서 작성한 목표·요건 전달]

AI : 요건을 분석해 P1-001~P1-015 backlog 후보를 도출합니다.
...
코드 개발 baseline도 비어 있으면 `Project Initialization`을 첫 P1 후보로 제안하고, 기능 후보는 사용자가 목표를 정한 뒤 `/register`로 추가한다.

```
AI : §2 목표가 확정되지 않아 기능 후보는 아직 Not Ready입니다.
     Implementation Baseline도 비어 있으므로 P1-001: Project Initialization을 먼저 제안합니다.
     기능 후보는 목표 확정 후 /register로 추가해주세요.
```
...
복잡하거나 여러 단계인 작업 → docs/works/phase1/P1-XXX-topic.md 생성 권장
                           착수 시: /work P1-XXX
```

Work 파일이 필요한 대표 케이스:
- Foundation 구조 설정 (P1-001)
- Security + 인증 구현 (P1-004)
- 서비스 규모가 큰 경우
- 프론트엔드 (기술 스택 결정 포함)
...
설정 변경               : 앱 기동 후 /actuator/health 확인
...
[ ] §4 Phase 1 Backlog
    ...
    [ ] AI: docs/backlog/PHASE1.md 후보 등록 (Done Criteria, Verification, Preconditions 포함)
...
| OQ-001 | Open   | WebClient vs OpenFeign       | P1-010 착수 전    |
```

AI는 관련 backlog 항목의 Preconditions에 `OQ-XXX 결정 필요`를 명시한다.
...
2. `/start` 또는 `/start` intent로 현재 상태를 확인한다.
...
6. 세션 종료 시 `/done` 또는 session summary intent로 검증 결과와 남은 리스크를 요약한다.
...
```bash
# 다음 세션에서
/start          # AI가 STATUS.md 기반으로 현재 상태 보고

/work P1-001    # 사용자가 착수할 작업 선택
