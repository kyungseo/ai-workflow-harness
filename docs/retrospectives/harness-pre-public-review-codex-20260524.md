...
> 작성일: 2026-05-24  
> 작성자: Codex  
> 범위: AI Workflow Harness v1.0.0 — HRN-028/029 이후 Codex/tool surface 및 scaffold 검증  
...
HRN-026~029 이후 harness는 Codex에서도 실행 가능한 형태로 정렬됐다. HRN-028에서 `.agents/skills/source-command-*`를 `harness-*`로 rename했고, HRN-029에서 product/harness track 양쪽 scope를 명확히 표현하기 위해 `workflow-*`로 재명명했다. `workflow-*` prefix는 일반 programming skill과 구분되며, skill이 harness workflow capability임을 정확히 드러낸다.

공개 전 남은 확인은 하나다.

1. 새 Codex 세션에서 `workflow-*` skill discovery/trigger가 실제 skill list에 반영되는지 확인
...
| HRN-028 후속 보강 | manual/prompt guide에 `.agents/skills/workflow-*`와 `.codex/hooks.json` 명시 | user-facing scaffold 설명과 실제 산출물 정렬 |
| HRN-029 | `harness-*` → `workflow-*` rename | product/harness track 양쪽 scope를 명확히 표현, harness-only 오해 제거 |
...
| Surface | 상태 | 비고 |
| --- | --- | --- |
| `AGENTS.md` | 정렬됨 | `.agents/skills/workflow-{name}/SKILL.md` mapping 명시 |
| `.agents/skills/workflow-*/SKILL.md` | 정렬됨 | 11개 skill, frontmatter name과 directory 일치 |
...
| `scripts/create-harness.sh` | 정렬됨 | `.agents/skills/*/` 동적 순회로 `workflow-*` 자동 반영 |

판단: Codex surface는 이제 "Claude command의 단순 복사본"이 아니라, workflow capability로 읽히는 이름 체계를 갖췄다.
...
| command/skill suffix mapping | 통과 | `.claude/commands/{name}.md` ↔ `.agents/skills/workflow-{name}/SKILL.md` 누락/초과 없음 |
...
| Codex command skills | 완료 | `workflow-*` 11개 |
| Skill naming hygiene | 완료 | 일반 skill과 구분되는 `workflow-*` prefix |
...
**Naming clarity**: `workflow-*`는 `work`, `doc`, `debug` 같은 generic skill name 충돌을 피하면서도 `source-command-*`보다 짧고, product/harness track 모두를 다루는 skill scope를 정확히 표현한다.
...
| Skill trigger false positive/negative | Medium | `workflow-*` prefix로 줄였지만 실제 trigger 품질은 사용 중 관찰 필요 |
...
3. `/start` intent는 `.agents/skills/workflow-start/SKILL.md`로 연결된다.
...
이 흐름은 Claude command와 의미상 같지만, Codex에서는 skill 이름이 `workflow-*`로 namespace화되어 있다는 점이 다르다. 이 차이는 올바른 방향이다.

---

## 8. 공개 전 권장 작업

1. 새 Codex 세션을 시작해 skill list에 `workflow-start`, `workflow-work`, `workflow-health`가 노출되는지 확인한다.
2. 새 세션에서 `/start` intent 또는 "현재 상태 요약" 요청이 `workflow-start` workflow를 자연스럽게 타는지 확인한다.
