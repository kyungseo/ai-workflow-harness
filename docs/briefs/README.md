...
| 2026-06-17 | [harness-distribution-positioning-20260617.md](harness-distribution-positioning-20260617.md) | harness source posture(public/private)와 adopter 진입 표면(직접/단일 distribution/변형 N개) 비교 | source는 public 유지 + adopter용 클린 distribution 표면 1개 추가가 권고. 이미 public·릴리즈된 상태라 private 회수는 이득 소진, 변형 N개는 auto-upgrade 부재로 영구 sync 세금 |
| 2026-06-22 | [dr-namespace-redesign-20260622.md](dr-namespace-redesign-20260622.md) | harness/product DR namespace 분리 — 4축 분해 + 옵션별 target-state mapping | 진짜 fork: ① high-band 유지(무투자, ai-deck 작업 보존) vs 구조 분리. 구조 분리는 ②b product-only prefix(DR/PDR) vs ③ asymmetric directory로 좁힘(②a full prefix는 framework rename 과투자 제외). ②b가 reference syntax·index 비용 0이라 우세 가능. ②b/③ 모두 ai-deck high-band 되돌림 매몰비용 발생. --check product tracking(B)은 기본 Deferred |
| 2026-06-22 | [rule-asset-generalization-strategy-20260622.md](rule-asset-generalization-strategy-20260622.md) | stack/safety rule 자산(java-spring/testing/infra/safety-critical) 일반화 + 멀티에이전트 option-pack 재설계 | 한 폴더에 두 축이 섞임. 축 A 안전(infra+safety-critical)은 source-first 정규화 — A1(always)/A2(path-scoped) 분할, Codex·AG는 `AGENTS.md → shared safety doc` 경로(AG runtime 검증 필요). 축 B stack(java-spring+testing)은 product-first로 spring-modular 정제 후 import. 단순 5파일 삭제는 비채택(안전 자산 손실) |
| 2026-07-13 | [harness-longterm-durability-review-20260713.md](harness-longterm-durability-review-20260713.md) | agent 도구 발전 속 harness 장기 내구성 — 4축 retain/refactor/replace 판정 | 전면 rewrite 불필요. ① policy retain / ② upgrade refactor(**provisional** — rfx generic 1건 feasibility, heterogeneous replay 전 확정 금지) / ③ projection refactor(rule surface 한정) / ④ enforcement 보완. 작업 가설(bounded heuristic): "deterministic contract·verification이 있을 때 agent 위임을 기본 후보로" |
