...
- **공통 기반:** `skills/workflow/`를 canonical 표준으로 정한 것과 **동일하게**, rule 자산도 `skills/{domain}/*.md`를 SSoT로 두고 툴별 surface는 thin adapter로 만든다. 현재는 canonical 없이 각 툴에 내용이 직접 박혀 중복된 상태다.
- **축 A — stack-agnostic 안전 레이어 (`infra` + `safety-critical`):** 이미 보편적이므로 **source-first**로 통합·정규화하고 **4개 툴 모두에 always-apply**로 깐다. 현재 Claude(`infra.md`, path-scoped)와 Cursor(`safety-critical.mdc`, always)에 **다른 메커니즘·다른 범위로 갈라져 있고 Codex/Antigravity엔 아예 없는** 비대칭이 핵심 결함이다. (Codex/AG는 always-rule surface가 없어 AGENTS.md/BEHAVIOR-PRINCIPLES 경로가 필요 — §3 참조.)
...
- 즉 Codex/Antigravity는 현재 java-spring/testing/infra/safety 규칙을 **전혀 받지 않는다.**
...
| 시급성 | 높음 (안전 공백) | 낮음 (option-pack 트리거 대기) |

### 2. 네이밍 재설계

| 현재 | 문제 | 후보 |
| --- | --- | --- |
| `testing` | 너무 광의 — 실제론 Spring/JUnit/Mockito 전용. generic repo에서 오해 | `spring-testing` / `jvm-testing` |
| `java-spring` | 비교적 명확하나 pack 체계 미정 | `spring-conventions` (pack: `spring-boot`) |
| `infra` (Claude) | always가 아니라 path-scoped라 경로 밖 공백 | 축 A로 흡수 |
| `safety-critical` (Cursor) | `infra`와 같은 의도 다른 이름 | **4툴 공통 명칭으로 통일** (예: `safety-critical`) |

### 3. 멀티에이전트 아키텍처 — `skills/` 표준 재사용

**기본 방향: `skills/workflow/`를 canonical 표준으로 정한 것과 동일하게, rule 자산도 `skills/{domain}/*.md`를 SSoT로 두고 툴별 adapter는 thin하게 만든다.** 현재는 canonical 없이 각 툴 surface에 내용이 직접 박혀 중복된 상태(workflow를 canonical화하기 전과 같은 문제)다.
...
- 안전/컨벤션 rule은 *always-apply / path-scoped* **적용**이다. Codex/Antigravity의 `.agents/skills/`는 "호출되는 skill"이지 "항상 적용되는 rule"이 아니다. 따라서 축 A 안전 레이어를 Codex/AG에 **상시** 적용하려면 skill이 아니라 **AGENTS.md 인라인 또는 `docs/BEHAVIOR-PRINCIPLES.md` 안전 절**로 가야 한다. (현재 BEHAVIOR-PRINCIPLES엔 `rm -rf`/`sudo` 같은 실행 guard가 없어 `safety-critical`은 독립 가치가 있다.)
...
- `skills/{domain}/convention.md`를 SSoT로 두면 **planning-pack 문서와 option-pack rule이 둘 다 거기서 파생**해 3-way 중복을 막는다 (같은 convention을 planning에선 문서로, execution에선 rule로 투영).
- 단 과도한 통합은 결합도를 높이므로, "code-conventions SSoT를 하나로 수렴 vs 분리 유지"는 축 B 착수 시 결정한다. **축 A(안전)는 planning-pack과 무관하므로 이 경계 논의에서 제외한다.**
...
1. **축 A를 먼저 닫는다 (독립, 시급).** `infra` + `safety-critical`을 stack-agnostic 안전 레이어로 통합하고, canonical + 4툴 adapter(Codex/Antigravity 포함)로 always-apply 정규화. → 안전 공백·툴 비대칭 해소. base-msa와 무관하므로 product 검증을 기다릴 이유 없음.
...
- 축 A 통합 명칭 확정 (`safety-critical` vs 신규)
- 축 A를 core 상시 포함으로 둘지, 아니면 always지만 `--no-safety` opt-out을 둘지
- 축 A의 Codex/Antigravity 적용 경로 (AGENTS.md 인라인 vs BEHAVIOR-PRINCIPLES 안전 절 vs 둘 다)
- `skills/{domain}/` 도메인 명칭 체계 (`skills/safety/`, `skills/spring/` 등)와 adapter pointer 형식
...
- 축 B code-conventions SSoT를 planning-pack과 수렴할지 분리할지 (§3b)
...
- 실제 안전 사고(무승인 destructive/infra/secret 노출)가 관측되면 → 축 A를 P1으로 승격
...
- 검증 영향: 현재 parity/mirror/invariant 테스트는 이 rule 파일들을 하드 참조하지 않음(`check-surface-mirror-parity.sh`는 command surface만 검사). 단 `docs/maintainer/VERIFICATION-COMMANDS.md` Layer U stack-marker는 `java-spring.md` 존재에 의존 → 축 B 변경 시 동반 갱신 필요.
- scaffold 영향: `scripts/create-harness.sh` line 544(`infra.md` 항상 복사), 558–561·665–668(spring-boot 분기), `docs/WORKFLOW-MANUAL.md` 296·821–823·844.
