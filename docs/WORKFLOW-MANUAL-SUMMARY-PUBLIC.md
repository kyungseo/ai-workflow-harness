...
- **Spring Boot MSA Template** 구축: AI Workflow의 적용과 운영을 테스트하기 위한 대상으로서의 부가 프로젝트
...
timeline
    ...
    Project Kickoff : First commit
                    : Product template 출발점 생성

    Product Foundation : Phase 1 기능 블록 완성
                       : README, architecture, status 문서 보강
                       : product delivery 중심 운영
...
> **Note** Repo 공개를 위해서는 `Spring Boot MSA Template`을 분리하는 절차가 필요하다. 
> 
> 이 과정을 거친 후 Private에서 Public으로 전환을 계획하고 있다.
...
| product 후보 선택 | `docs/backlog/PHASE{n}.md` |
...
flowchart TD
    ...
    NOW -- "아니오" --> TYPE{"성격"}
    TYPE -- "Product / Phase 기능" --> PHASE["docs/backlog/PHASE{n}.md"]
...
| product 후보 | `docs/backlog/PHASE{n}.md` |
...
|-------------------------------| --- | --- | --- |
| L1 product surface (제품 영역)    | 간단 plan 승인 후 실행. Work 파일 없이 Quick Mode 가능 | Work checkpoint/discovery는 승인 불필요, 실행 후 대상 Work ID와 변경 보고 | validation 결과, diff summary, 제안 commit message 보고 후 승인 |
...
Quick Mode는 product surface의 작고 명확한 L1 작업에 한정한다.
