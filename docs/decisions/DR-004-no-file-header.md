...
파일 헤더를 추가하지 않는다. `LICENSE.txt` 파일이 전체 프로젝트의 라이선스를 커버한다.
...
| 헤더 없음 (채택) | AI 컨텍스트 최소화, diff 노이즈 없음, 소급 불필요 | 파일 단위 라이선스 표시 없음 (루트 라이선스 파일로 대체) |

## Rationale

- 오픈소스 법적 효력은 루트 라이선스 파일 하나로 전체 프로젝트를 커버한다 (Berne Convention 기준).
...
Low — LICENSE.txt 파일이 있으므로 헤더를 나중에 추가해도 법적 효력에 영향 없음.
