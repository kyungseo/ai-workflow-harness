...
Track: harness
Amended: product 작업 branch 단축 패턴을 `feature/p{n}-{topic}` → `feature/prod-{topic}`로 정정(phaseless 정합). 결정 방향 불변.
...
**5. harness "Phase" 보존.** 명령 절차 단계(work-doc/repo-health "Phase 1-6"), STATUS 필드(`Current phase`), harness 리팩토링 milestone("Phase 2")은 변경하지 않는다. 이 결정은 product track backlog/work 네이밍에만 적용된다. (`Current phase`의 descriptive/optional de-formalization은 DR-032 — 본 결정은 네이밍만 다룬다.)
