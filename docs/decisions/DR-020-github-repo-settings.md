...
- 2026-06-06 handoff note: required check로 연결된 workflow에 path filter가 있으면 filter 미일치 PR에서 check가 실행되지 않아 `Expected`/pending 상태로 남을 수 있다. (d) source-gitflow environment bootstrap은 `harness-validate` 연결 시 항상-실행 gate job 또는 path filter 없는 status-check 패턴을 함께 결정해야 한다.
