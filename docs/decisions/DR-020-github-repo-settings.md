...
| 항목 | 현재 상태 | 결정 필요 이유 |
|------|-----------|----------------|
| Merge 방식 제한 | 3개 모두 허용 (merge, squash, rebase) | DR-017은 regular merge 기본·squash 선택을 정했으나, ruleset에서 rebase를 명시적으로 차단할지 미결. rebase 허용 시 선형 히스토리 유지는 가능하지만 DR-017 방침과 어긋남 |
...
- Merge 방식 제한 미결 상태 — rebase merge를 차단할 시점에 ruleset 재수정 필요.
