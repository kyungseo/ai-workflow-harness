...
Date: 2026-05-25
Status: Accepted (일부 항목 Deferred · Amended 2026-06-08)
...
| 항목 | 값 |
|------|----|
| `delete_branch_on_merge` | **false** (2026-06-08 변경) — auto-delete가 develop→main 릴리즈 merge 시 영구 브랜치 `develop`을 삭제하는 위험. feature 브랜치는 `gh pr merge --delete-branch` 명시 플래그로 정리 |
| `allow_update_branch` | true — PR에서 "Update branch" 버튼 활성화 |
| `has_discussions` | true — public 커뮤니티 피드백 채널 |

> **2026-06-08 Amended:** `delete_branch_on_merge`를 `true` → `false`로 전환했다. v1.1.0 릴리즈(develop→main PR #112) merge 시 GitHub의 auto-delete-head-branch가 발동해 영구 통합 브랜치 `develop`을 원격에서 삭제했고, `protect-develop` ruleset의 `deletion` 보호도 이를 막지 못했다(auto-delete가 ruleset deletion 규칙을 우회). post-merge sync의 `git push origin develop`가 우연히 develop을 재생성해 사고를 피했으나, sync 단계를 누락하면 develop 유실로 이어진다. Gitflow의 develop/main은 영구 브랜치이므로 auto-delete를 끄고, 일회성 feature 브랜치만 `--delete-branch` 명시 플래그로 정리하는 것이 안전하다.
...
| 항목 | 결정 | 결정일 | 근거 |
|------|------|--------|------|
| Merge 방식 제한 | ruleset 차단 미적용. DR-017 운영 정책 준수 (feature→develop: Squash 기본, Regular 예외; develop→main: Regular Merge 원칙; rebase 비권장) | 2026-05-29 (Amended 2026-06-07) | rebase 차단 시 관리 복잡도 증가 대비 실익이 적음. solo 운영 환경에서 DR-017 운영 정책으로 충분히 제어 가능 |
...
| squash only | 히스토리 일관성 최대 | develop→main 커밋 맥락 손실 |
...
- feature 브랜치는 `gh pr merge --delete-branch` 명시 플래그로 정리(repo auto-delete는 off). develop/main 영구 브랜치는 릴리즈 merge 시에도 보존됨.
- 시크릿이 포함된 커밋은 push 시점에 차단됨.
- Merge 방식 제한: ruleset 차단 없음. DR-017 운영 정책 (feature→develop: Squash 기본 / Regular 예외; develop→main: Regular Merge 원칙; rebase 비권장)으로 준수.
