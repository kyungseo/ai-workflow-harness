...
| 2026-06-10 | optional-pack/onboarding 문서를 audience별 디렉토리로 **물리 이동**하자는 제안 검토 — 유보 유지. ~50 참조 + scaffold adapt/generated-README/invariants test rewiring으로 reversal cost 큼이 재확인됨. 단 "reversal cost가 높다"는 논거가 **hardcoded flat-path 참조 방식에 종속**된다는 재검토 의문은 타당 → manifest/anchor indirection과 함께 P1 "optional pack 재정의"에서 재고하도록 라우팅. **결정(물리 이동 보류) 불변** — CHORE-20260610-009 |
