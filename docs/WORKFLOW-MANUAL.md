...
| Status Always Current | 작업 상태가 바뀌면 State Update Gate → 승인 → 필요한 상태 파일 갱신 순서를 따른다 |
...
| 개발자 + AI 겸용 | `PLAN-SUMMARY.md`, `WORKFLOW-MANUAL.md`, `GIT-WORKFLOW.md`, `PLAN.md` |
...
- State Update Gate에 맞는 제안을 먼저 보고하고 사용자 승인을 받는다
...
4. State update: Active Work 포인터 제거 제안
5. Work 파일은 즉시 archive하지 않는다.
...
| Layer | 변경 유형 | Gate |
| --- | --- | --- |
| Layer 1 — Work 파일 | Checkpoint 상태 업데이트, Discovery 추가 | 승인 불필요. 실행 후 대상 Work ID와 변경 내용 보고 |
...
flowchart TD
    ...
    STATE_NEED -- "Work Done / STATUS pointer" --> STATE_PROPOSAL["State Update Gate\n대상 Work ID 명시"]
...
- `docs/works/{category}/README.md` Candidate 또는 Active 섹션에 항목 추가
- 착수 승인 시 STATUS.md Active Work에 Work 파일 pointer 추가 제안

**cascade:** Work 파일과 category index를 갱신한다. 착수 상태가 되면 STATUS Active Work pointer를 State Update Gate에 따라 제안한다.
...
**cascade:**
- 산출물이 현재 상태 추적에 필요하면 State Update Gate에 맞는 제안
...
이 정보를 바탕으로 아래 파일을 채울 내용을 State Update Gate에 맞게 제안해줘.
...
파악한 내용을 바탕으로 아래를 State Update Gate에 맞게 제안해줘:
