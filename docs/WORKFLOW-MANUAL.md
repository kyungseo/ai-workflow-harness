...
| Status Always Current | 작업 상태가 바뀌면 STATUS Update Proposal → 승인 → docs/STATUS.md 갱신 순서를 따른다 |
...
graph TD
    ...
    COMMANDS --> CMD_D["/done · /health<br/><i>세션 종료·점검</i>"]
...
flowchart TD
    ...
    TYPE -- "Product · MSA 기능" --> PBL["backlog/PHASE{n}.md\n← P{n}-NNN"]
...
- `STATUS Update Proposal`을 먼저 보고하고 사용자 승인을 받는다
...
하네스 프로토콜의 카테고리별 상세 문서. session state machine, context loading, work item routing, document lifecycle, trigger/cascade, recovery/validation을 분리한다. 전체 일괄 로드는 금지하고 필요한 파일만 선택한다.
...
flowchart TD
    ...
    RECOVER --> PLAN
    PASS -- 통과 --> STATUS_NEED{STATUS.md 변경 필요?}
    STATUS_NEED -- 예 --> STATUS_PROPOSAL["STATUS Update Proposal\n변경 섹션·이유·되돌리기 비용"]
    STATUS_PROPOSAL --> STATUS_APPROVAL{사용자 승인?}
    STATUS_APPROVAL -- 수정 요청 --> STATUS_PROPOSAL
    STATUS_APPROVAL -- 승인 --> CHECKPOINT_UPDATED["CHECKPOINT\nSTATUS.md 갱신\n필요 시 DR/TODO/문서 cascade"]
    STATUS_NEED -- 아니오 --> CHECKPOINT_CLEAN["CHECKPOINT\n검증 결과·문서 cascade 확인"]
    ...
    style STATUS_APPROVAL fill:#fff3cd,stroke:#ffc107
```
...
sequenceDiagram
    ...
    alt 검증 실패
        ...
        C->>U: STATUS Update Proposal<br/>(필요한 경우)
        U->>C: STATUS 변경 승인 또는 생략 확인
        C->>F: CHECKPOINT - 승인된 경우 docs/STATUS.md 갱신
...
flowchart TD
    ...
    ROUTE -- "Product / Phase{n} 작업" --> PBACKLOG["docs/backlog/PHASE{n}.md"]
...
| `harness-protocol/*.md` | 특정 카테고리 규칙 수정·상세 검토 | 전체 일괄 로드 금지 |
...
승인된 scope 안에서 L1 변경은 빠르게 진행할 수 있다. 다음 두 경우에는 먼저 보고하고 사용자 승인을 기다린다.

| 상황 | Gate |
| --- | --- |
| 승인된 scope 밖 파일(README, STATUS, workflow 문서, commands, rules 등)로 확장 | 추가 scope·이유·검증 방법 보고 → 승인 후 진행 |
| 모든 변경의 commit 전 | validation 결과·diff summary·commit message 보고 → 승인 후 commit |

> 상세 규칙: `docs/AGENT-WORKFLOW.md` "Scope And Commit Approval" 섹션
...
> 각 트리거의 발동 조건·cascade 상세는 아래 T1~T9 개별 섹션을 참조한다.
...
| T7 | Harness protocol 업데이트 | docs/AGENT-WORKFLOW.md 워크플로우 규칙 변경 / `.claude/commands/*.md` 변경 / trigger set(T1~T9) 추가·변경 | Claude 능동 제안 | `HARNESS-PROTOCOL.md`, `docs/harness-protocol/`, 필요 시 `WORKFLOW-MANUAL.md` |
...
- trigger set(T1~T9) 추가·변경

**cascade (섹션별):**

| 변경 원인 | 확인 대상 섹션 |
|-----------|----------------|
| 컨텍스트 로드 조건 변경 | `docs/harness-protocol/02-context-loading.md`, 필요 시 이 문서 §4-3-B |
...
| 트리거 추가·변경 | `docs/harness-protocol/05-triggers-and-cascade.md`, 필요 시 이 문서 §7 |
...
`scripts/create-harness.sh`로 하네스 파일 구조를 자동 생성한다. 기본값은 언어·프레임워크를 가정하지 않는 `generic` profile이며, 이 템플릿의 Java/Spring Boot 보조 규칙과 prompt가 필요할 때만 `--profile spring-boot`를 사용한다. Codex는 `AGENTS.md`를 기본 진입점으로 쓰며, `codex-session-start.md`는 수동 bootstrap이 필요한 환경의 fallback이다.
...
| `.cursor/config.json`, `.cursor/rules/*.mdc` | 복사 | 기본 profile은 generic rules와 `safety-critical.mdc` 포함 |
