...
| `java-spring.md` | services/, gateway/, common/ | Java 21, Spring Boot 3.5.x 컨벤션, MyBatis #{} |
...
| `/pick` | 다음 작업을 선택할 때 | backlog 후보 비교, 우선순위 추천, 구현 전 승인 대기 |
| `/work {ID}` | 특정 작업을 시작할 때 | 해당 backlog 항목 계획 수립, "진행할까요?" 후 대기 |
| `/resume {ID}` | 중단된 작업을 재개할 때 | 파일 상태 vs STATUS.md 비교 후 계속 진행 |
| `/debug` | 버그 분석/수정 시 | 코드·로그·테스트 근거로 원인 파악, 최소 변경 계획 |
| `/done` | 세션 종료 시 | 완료 작업, 변경 파일, 검증 결과, 리스크, 다음 세션 primer 요약 |
...
- [ ] **`.claude/commands/`** 파일 6개 복사 및 프로젝트 Phase명 조정
  - `start.md`, `pick.md`, `work.md`, `resume.md`, `debug.md`, `done.md`
