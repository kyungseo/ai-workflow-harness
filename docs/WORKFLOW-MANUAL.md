...
> 2026-05-14 기준 `docs/CLAUDE.md`, `.claude/commands/*`, `.claude/rules/docs-workflow.md`는 product/harness backlog 분리와 상태 머신 게이트를 반영한다.
...
| AI 운영 (Claude) | `CLAUDE.md`, `STATUS.md`, `HARNESS-PROTOCOL.md`, `HARNESS-QUICK-REFERENCE.md`, `harness-protocol/`, `PLAN-SUMMARY.md`, `backlog/`, `decisions/`, `archive/`, `TODO/` |
...
자동 로드되는 프로젝트 운영 규칙. 세션 시작, context routing, risk gate, STATUS 규칙, verification defaults, Project Constants만 짧게 정의한다.
...
핵심은 `CLAUDE.md`와 `docs/CLAUDE.md`는 작게 유지하고, 실제 하네스 운영 규칙은 `docs/HARNESS-PROTOCOL.md`와 `docs/harness-protocol/`로 분리하는 것이다.
...
- CLAUDE.md (루트, 영어)
- docs/CLAUDE.md (한국어, 자동 로드 최소 운영 규칙)
...
| T7 | Harness protocol 업데이트 | docs/CLAUDE.md 워크플로우 규칙 변경 / `.claude/commands/*.md` 변경 / T1~T6 트리거 추가·변경 | Claude 능동 제안 | `HARNESS-PROTOCOL.md`, `docs/harness-protocol/`, 필요 시 `WORKFLOW-MANUAL.md` |
...
*이 문서는 사용자 매뉴얼이다. Claude instruction은 `CLAUDE.md`와 `docs/CLAUDE.md`, 하네스 실행 프로토콜은 `docs/HARNESS-PROTOCOL.md`와 `docs/harness-protocol/`을 따른다.*
