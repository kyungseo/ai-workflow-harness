...
Work 파일(`docs/works/{category}/{ID}-{topic}.md`)은 **큰 작업 하나의 SSoT**다. Plan, Done Criteria, Verification, Checkpoints, Discovery를 한 파일에 담는다. STATUS.md는 dashboard(pointer), Work 파일은 그 작업의 상세다. 일반 작업은 backlog와 STATUS.md만으로 충분하고, Work 파일은 backlog를 대체하지 않는다.
