...
> **Note** 여기서 말하는 '하네스'는 AI가 프로젝트 안에서 제멋대로 움직이지 않도록, 상태와 승인과 검증 흐름을 감싸는 운영 뼈대라는 뜻이다. 이 문서는 그 구조를 사용자가 이해하고 운영할 수 있게 설명하는 **사용자용 종합 가이드**다.
...
이 문서는 그 규칙을 사용자가 **읽고 판단하고 운영하는 방법**을 설명한다. 핵심만 빠르게 잡고 싶다면 먼저 `README.md`를 보고, scaffold 직후 첫 세션을 막 시작하는 단계라면 `docs/SCAFFOLD-ONBOARDING-GUIDE.md`를 먼저 읽는 편이 낫다.

이 문서는 특히 아래 상황에서 유용하다.

- 이 workflow가 왜 필요한지, 어떤 문제를 줄이려는지 이해하고 싶을 때
- `/session-start`, `/work-plan`, `/work-close` 같은 흐름이 어떻게 이어지는지 사람 관점에서 파악하고 싶을 때
- STATUS.md, backlog, Work 파일, DR이 서로 어떻게 역할을 나누는지 헷갈릴 때

반대로, 지금 당장 **정확한 실행 절차**가 필요하다면 이 문서보다 `docs/AGENT-WORKFLOW.md`, `docs/HARNESS-PROTOCOL.md`, `skills/workflow/*.md`가 우선이다. 이 매뉴얼은 그 절차를 다시 SSoT로 만들지 않는다.
...
> **전제:** 본 매뉴얼의 내용은 'Claude'를 기준(예시)으로 기술한다. Codex, Antigravity, Cursor 사용 시 해당 도구의 진입점과 설정을 참조하도록 한다. Antigravity(Gemini 기반)는 별도 진입 파일 없이 root `AGENTS.md` + `.agents/skills/`(Codex surface)를 자동 소비한다.
...
AI 도구는 강력하지만, 프로젝트 운영 방식이 정리되어 있지 않으면 비슷한 문제가 반복된다. 세션이 바뀔 때마다 같은 설명을 다시 해야 하고, 승인 없이 범위가 커지고, 왜 그런 결정을 했는지가 대화 속에만 남는다.

이 구조는 그런 문제를 줄이기 위해 설계되었다.
...
| 목적 | 먼저 볼 곳 |
| --- | --- |
| scaffold 직후 첫 부팅 | [docs/SCAFFOLD-ONBOARDING-GUIDE.md](SCAFFOLD-ONBOARDING-GUIDE.md) 또는 [Appendix B. New Project Initialization](#appendix-b-new-project-initialization) |
...
처음 읽는 순서를 하나만 추천하면 이렇다.

1. 이 문서의 `Overview`
2. `§4 Workflow Diagrams`
3. `§5 Slash Commands Reference`
4. 필요할 때만 `§7 Trigger Reference`
...
- **프롬프트** `prompts/` — session-start fallback 템플릿.
...
| AI 운영 공통 | `docs/BEHAVIOR-PRINCIPLES.md`, `docs/AGENT-WORKFLOW.md` (Claude Code: `CLAUDE.md` → 자동 import / Codex·Antigravity: `AGENTS.md` → 위임 / Cursor: `.cursor/rules/*.mdc`) |
...
| 역사·평가 | `docs/archive/`, `docs/retrospectives/`, `docs/briefs/`, reference-only plan |
...
| --- | --- | --- |
| Product track | 실제 제품/서비스/콘텐츠의 기능, 테스트, 문서, 인프라 work | `docs/backlog/PRODUCT.md`, `docs/works/product/` |
...
flowchart TD
    ...
    TYPE -- "Product track 기능" --> PBL["backlog/PRODUCT.md\n(Work ID: FEAT-YYYYMMDD-NNN, 착수 시 확정)"]
...
> 위 STATUS.md, PLAN-SUMMARY.md, HARNESS-ARCHITECTURE.md, HARNESS-MAINTAINER-GUIDE.md, HARNESS-PROTOCOL.md, HARNESS-QUICK-REFERENCE.md, backlog, decisions, archive, retrospectives, briefs 등 **개별 파일의 역할**은 source repo README의 Key Documents가 SSoT다.
...
sequenceDiagram
    ...
    C->>F: PLAN - 작업ID에 따라 PRODUCT.md 또는 HARNESS.md 읽기
...
flowchart LR
    ...
    subgraph AG ["Antigravity (Gemini 기반)"]
        direction TB
        AG1["📄 AGENTS.md\n세션 시작 시 자동 로드"]
        AG2["📁 .agents/skills/\nCodex surface 자동 소비"]
        AGF["📄 codex-session-start.md\nfallback prompt 공유"]
        AG1 --> AG2
        AGF -.-> AG1
    end
...
flowchart TD
    ...
    ROUTE -- "Product track 작업" --> PBACKLOG["📄 docs/backlog/PRODUCT.md"]
...
| `backlog/PRODUCT.md` | product 준비 작업 선택 | harness 작업 |
...
Claude Code에서 `/명령명`으로 호출한다. 상세 절차는 `skills/workflow/{name}.md`가 canonical SSoT이며, `.claude/commands/*.md`, `.agents/skills/workflow-*/SKILL.md`, `.cursor/rules/workflow.mdc`는 tool-specific adapter다. Antigravity는 Codex와 동일하게 `.agents/skills/workflow-*/SKILL.md`를 자동 소비한다(별도 adapter 없음).
...
| Utility / Analysis | `/work-register [설명]` | 새 작업 항목을 등록할 때 | 긴급도·성격 판단 → STATUS Active Work / Next Actions / PRODUCT.md / HARNESS.md 중 라우팅 → state-change proposal(필요 시) → 긴급 항목이면 /work-plan 연결 제안 |
| Utility / Analysis | `/work-debug` | 버그 분석/수정 시 | 코드·로그·테스트 근거로 원인 파악, 최소 변경 계획 |
| Utility / Analysis | `/work-brief [topic]` | 방향 비교 문서를 만들거나 문서 분류를 바로잡을 때 | brief/retrospective/decision triage → 관련 문서 후보 축소 → `docs/briefs/` 작성 또는 live 경로 재분류 → index/link/cascade 검증 |
...
# 방향 비교 / 문서 분류
/work-brief "이 문서가 회고보다 brief에 가깝다. 분류와 연결을 정리"
...
flowchart LR
    ...
    ACCEPT --> ROUTE{영향 범위}
    ROUTE -- Product track --> PBACKLOG["📄 docs/backlog/PRODUCT.md\nDone Criteria 조정"]
...
| phase/milestone 전환 선언 | STATUS Recent Decisions 기록, (해당 시) archive 이동·PLAN 재편 |
...
| 원인 불명의 이슈 해결 | troubleshooting 기록 |
...
## Appendix A. Prompt Fallback Usage

### Structure

```
prompts/
├── README.md                    ← session-start fallback guide
├── claude-session-start.md      ← Claude fallback 세션 부트스트랩
├── codex-session-start.md       ← Codex / Antigravity fallback 세션 부트스트랩
...
| 상황 | 추천 진입점 |
| --- | --- |
| Claude Code slash command 사용 가능 | `.claude/commands/session-start.md` 기반 `/session-start` |
| Codex adapter 사용 가능 | `.agents/skills/workflow-session-start/SKILL.md` |
| Antigravity 사용 가능 | root `AGENTS.md` 자동 로드 + `.agents/skills/workflow-session-start/SKILL.md` 자동 소비 (intent: `/session-start`) |
| Cursor session bootstrap | `prompts/cursor-session-start.md` |
| command/adapter 사용 불가 | 해당 도구의 `*-session-start.md` fallback prompt |

### Usage

1. 가능하면 도구별 command/adapter로 세션을 시작한다.
2. command/adapter를 사용할 수 없으면 `prompts/README.md`에서 fallback prompt를 확인한다.
3. 해당 `*-session-start.md` 파일을 열어 현재 repo path, branch, active work, review gate 정보를 채운다.
4. AI 도구에 붙여넣고 loaded docs와 현재 상태 요약을 확인한다.

기존 generic task prompt와 Spring/profile prompt 예시는 live prompt library에서 제외하고 `docs/archive/prompts/`에 보존한다. Archive 파일은 역사적 참고용이며 scaffold target prompt surface로 취급하지 않는다.
...
- [ ] **`docs/AGENT-WORKFLOW.md`** (한국어) 생성
  ...
  - Context Routing (`PRODUCT.md` vs `HARNESS.md`)
...
- [ ] **`docs/works/product/`** Work 파일 생성 여부 결정
  - 기본값은 생성하지 않는다
...
- [ ] **canonical `skills/workflow/` 1벌 + tool adapter** 복사 및 프로젝트 Phase명·backlog 경로 조정
  ...
  - adapter: `.claude/commands/{name}.md`(Claude), `.agents/skills/workflow-{name}/SKILL.md`(Codex; Antigravity가 동일 surface 재사용), `.cursor/rules/workflow.mdc`(Cursor)
  - 새 프로젝트는 `PRODUCT.md`를 product backlog로 사용
  - harness 작업은 `docs/backlog/HARNESS.md`를 사용하도록 유지
  - 복사한 command/rule 안의 프로젝트명, Phase명, verification command를 점검

#### Step 6 — Session-start fallback prompt

- [ ] **`prompts/`** 디렉토리 복사
  - `claude-session-start.md`, `codex-session-start.md`(Antigravity도 이 fallback 공유), `cursor-session-start.md` 복사 후 프로젝트명 수정
  - `README.md` 업데이트
  - 반복 실행 절차와 중복되는 항목은 `.claude/commands/` 또는 `.agents/skills/`로 옮길지 별도 검토
