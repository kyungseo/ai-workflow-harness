...
cascade 흐름(어떤 변경이 어떤 문서를 연쇄 업데이트하는가)은 [§10 Trigger Cascade Overview](#10-trigger-reference)를 참조한다.
...
flowchart TD
    ...
    ON_DEMAND -- "전체 근거 필요" --> PLAN["docs/PLAN.md\n명시적 요청 시만"]
    ON_DEMAND -- "이전 Phase 맥락" --> ARCHIVE["docs/archive/\n이력 복원 시만"]
    ON_DEMAND -- "세부 분해 확인" --> TODO["docs/TODO/PHASE{n}/\n세부 서브태스크 시만"]
...
├── codex-session-start.md       ← Codex 세션 부트스트랩
