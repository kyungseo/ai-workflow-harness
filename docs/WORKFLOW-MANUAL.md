...
graph TD
    ...
    COMMANDS --> CMD_A["Session lifecycle<br/>/session-start · /work-select · /session-summary"]
    COMMANDS --> CMD_B["Work lifecycle<br/>/work-plan · /work-resume · /work-close"]
    COMMANDS --> CMD_C["Utility / Analysis<br/>/work-register · /work-debug · /work-doc<br/>/repo-decision · /repo-health"]
...
> `/work-resume {ID}` 실행 시 이 규칙이 자동으로 적용된다. → [§5 /work-resume 참조](#5-slash-commands-reference)
...
세션 중 빠르게 확인하는 실행 카드. `/session-start`, `/work-plan`, `/work-close`, `/session-summary` 흐름에서 필요한 최소 규칙, risk level, validation, naming 규칙을 담는다. 상세 판단이 필요하면 `docs/HARNESS-PROTOCOL.md`로 올라간다.

### `docs/backlog/PHASE{n}.md`

적용 대상 프로젝트의 Product track 후보 작업 목록. 우선순위(P0~P3), 선행 조건, Done Criteria, Verification을 포함한다. `/work-select` 명령에서 product/phase 작업을 고를 때의 입력 소스다.
...
1. `/work-plan {ID|title-or-slug}` 실행 — Claude가 확정 Work ID 또는 제목/slug로 backlog/Work 파일을 찾고, Work ID가 없으면 착수 승인 시 확정한다
...
**완료 절차 (Done) — `/work-close`:**

Work를 완료하면 `/work-close`를 실행한다. 세션은 종료되지 않고 계속된다.
`/work-close`는 Work Done 처리이며 commit/PR 전 STATUS Finalization Gate나 Tracking Finalization Gate를 대체하지 않는다.
Review-sensitive Work는 사용자 최종 리뷰를 Done Criteria에 선택 포함한다. `/work-close`는 모든 Work에 사용자 리뷰를 강제하지 않지만, Done Criteria에 명시된 리뷰 조건은 Done 처리 전 반드시 확인한다.
...
5. Archive 여부 선택: 지금 바로 archive하거나 `/session-start`·`/work-resume`까지 보류

**세션 종료 — `/session-summary`:**

`/session-summary`은 Work Done 처리를 포함하지 않는다. Work를 완료했다면 `/work-close`를 먼저 실행한다.
`/session-summary`은 세션 요약만 출력하며, Active Work가 있으면 Discovery 미기록 여부를 확인하고 기록할 내용을 제안한다.

**Archive 절차 (Done → Archived):**

Archive는 사용자가 명시적으로 승인했거나 `/session-start`·`/work-resume`에서 Done 항목 발견 후 승인한 경우에만 수행한다.
...
**로드 조건:** `/work-resume` 또는 Active Work의 세부 Checkpoint 확인 시.
...
- `hooks.Stop` — 세션 종료 전 `/session-summary` 절차 점검 reminder (단, `/exit` 직접 입력 시에는 Claude 응답 생명주기를 거치지 않아 발동하지 않음 — `/session-summary` 실행 후 종료 권장)
...
`/work-plan`가 계획 수립 전에 자동으로 판단하는 위험도 기준이다.
...
|----|------|-----------|------|-------------------|
| T1 | DR 생성 | 계획 승인 직후 / OQ Closed 전환 / `/session-summary` 실행 | Claude 능동 제안 | `docs/STATUS.md` Blockers/OQ |
...
| T9 | 발표/보고 산출물 생성 | `/work-doc` 실행 또는 발표·보고·review package 생성 요청 | Claude 능동 제안 | `docs/presentations/`, `docs/reports/`, source traceability, STATUS/backlog 참조 필요 여부 |
...
- `/session-summary` 실행 시 세션 중 확정된 결정 검토 단계
...
**발견 시점:** `/repo-health` E영역 DR 위생 점검 시 후보 식별 → cascade 대상 목록 함께 제시
...
**중요:** PLAN.md 문서 현행화 작업 자체는 DR 기록 불필요 — 기존 DR 결정의 결과를 문서에 반영하는 것이므로 `/session-summary` 시 DR 제안 대상에서 제외한다.
...
Codex는 `AGENTS.md`를 기본 진입점으로 쓰고 첫 요청은 `/session-start` intent로 시작하며, `codex-session-start.md`는 수동 bootstrap이 필요한 환경의 fallback이다.
...
docs/backlog/PHASE1.md ← baseline 완료 후 도출한 초기 작업 항목 (Work ID는 /work-plan 착수 시 확정)
```
...
- `docs/GIT-WORKFLOW.md` — Gitflow 브랜치 전략, Branch Isolation Rule, Release Cycle 정의 (`policy_type: source-gitflow` marker로 `/work-plan`, `/work-close` Branch Isolation Gate 활성화)
...
- docs/backlog/PHASE1.md: Implementation Baseline 완료 후 도출할 초기 작업 후보 (Work ID는 /work-plan 착수 시 확정)
...
```bash
/session-start          # STATUS 확인 및 현재 상태 요약
/work-select           # 첫 작업 후보 추천
/work-plan [title-or-slug]    # 선택한 작업 계획 수립 (Work ID는 착수 승인 시 확정)
```
...
- docs/backlog/PHASE1.md: 남은 product 작업 후보 (Work ID는 /work-plan 착수 시 확정)
- docs/backlog/HARNESS.md: AI workflow와 example pack 정비 후보
- docs/AGENT-WORKFLOW.md: Project Constants, Verification Defaults
...
Claude가 코드베이스를 파악하고 state-change proposal을 제시하면 승인한다. 이후 `/work-select`으로 작업을 선택해 바로 이어갈 수 있다.
