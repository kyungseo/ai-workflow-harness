...
> 현재 하네스 재편 계획과 rollback 기준은 `docs/HARNESS-REFACTOR-PLAN.md`를 따른다.
> 2026-05-14 기준 `docs/AGENT-WORKFLOW.md`, `.claude/commands/*`, `.claude/rules/docs-workflow.md`는 product/harness backlog 분리와 상태 머신 게이트를 반영한다.
...
3. [Component Role Reference](#3-component-role-reference) · [3-0. Document Ecosystem Map](#3-0-document-ecosystem-map)
...
7. [Prompt Library Usage](#7-prompt-library-usage)
8. [New Project Initialization](#8-new-project-initialization)
9. [Language Rules Summary](#9-language-rules-summary)
10. [Trigger Reference](#10-trigger-reference)
...
graph TD
    ...
    ROOT --> AGENTS_ROOT["📄 AGENTS.md<br/><i>Codex 진입점 (영어)</i>"]
    ROOT --> DOCS["📁 docs/"]
    ROOT --> DOT_CLAUDE["📁 .claude/"]
    ROOT --> PROMPTS["📁 prompts/"]
    ROOT --> CURSOR["📁 .cursor/ (선택)"]

    DOCS --> AGENT_WORKFLOW["📄 AGENT-WORKFLOW.md<br/><i>공통 운영 규칙 (한국어)</i>"]
    DOCS --> STATUS["📄 STATUS.md<br/><i>라이브 상태판</i>"]
    DOCS --> HARNESS_PROTOCOL["📄 HARNESS-PROTOCOL.md<br/><i>하네스 운영 프로토콜</i>"]
    DOCS --> HARNESS_QUICK["📄 HARNESS-QUICK-REFERENCE.md<br/><i>실행 퀵 레퍼런스</i>"]
    DOCS --> PLAN_S["📄 PLAN-SUMMARY.md<br/><i>기술 스택·아키텍처 요약</i>"]
    DOCS --> PLAN["📄 PLAN.md<br/><i>전체 기술 근거 (필요 시만)</i>"]
    DOCS --> HARNESS_DETAIL["📁 harness-protocol/<br/>01~06 상세 규칙"]
    DOCS --> BACKLOG["📁 backlog/<br/>PHASE{n}.md / HARNESS.md"]
    DOCS --> DECISIONS["📁 decisions/<br/>DR-*.md"]
    DOCS --> ARCHIVE["📁 archive/<br/>phase{n}-status.md<br/>harness-refactor-YYYYMMDD/"]
    DOCS --> RETRO["📁 retrospectives/<br/>harness-evaluation-*.md"]
    DOCS --> TROUBLE["📁 troubleshooting/<br/>증상→원인→조치 기록"]
    DOCS --> TODO["📁 TODO/<br/>PHASE{n}/<br/>{BACKLOG-ID}-{topic}.md"]
    ...
    CURSOR --> CURSOR_RULES["📁 rules/<br/>*.mdc"]

    style CLAUDE_ROOT fill:#ffeeba,stroke:#ffc107
    style AGENTS_ROOT fill:#ffeeba,stroke:#ffc107
    style STATUS fill:#d4edda,stroke:#28a745
    style HARNESS_PROTOCOL fill:#d4edda,stroke:#28a745
    style HARNESS_QUICK fill:#d4edda,stroke:#28a745
...
| 분류 | 파일 | Claude 로드 방식 |
| --- | --- | --- |
| 항상 자동 로드 | `CLAUDE.md`, `docs/AGENT-WORKFLOW.md` | 세션 시작 시 자동 |
| 세션마다 수동 로드 | `docs/STATUS.md` (상단만) | `/start` 시 명시적 로드 |
| 하네스 작업 시 로드 | `docs/HARNESS-PROTOCOL.md`, `docs/HARNESS-QUICK-REFERENCE.md` | workflow/harness 변경 시 |
...
graph TD
    ...
    subgraph WORK ["작업 관리"]
        ...
        HB["docs/backlog/HARNESS.md\nHarness 후보 작업"]
        DR["docs/decisions/DR-*.md\n결정 기록"]
    end

    subgraph HPROTO ["하네스 운영 프로토콜"]
        HP["docs/HARNESS-PROTOCOL.md\n상태 머신 기준"]
        HQ["docs/HARNESS-QUICK-REFERENCE.md\n빠른 실행 규칙"]
        HD["docs/harness-protocol/\n상세 규칙"]
        WM["docs/WORKFLOW-MANUAL.md\n사용자 매뉴얼"]
    end

    subgraph TOOLS ["도구별 실행 표면"]
        CMD[".claude/commands/\n/start /pick /work /doc /done"]
        RULE[".claude/rules/\n세부 guardrail"]
        CUR[".cursor/rules/\nCursor 정합성"]
    end
...
다른 프로젝트에서도 재사용 가능하게 유지한다. `@docs/AGENT-WORKFLOW.md`로 프로젝트 규칙을 import한다.
...
상세 하네스 규칙은 `docs/HARNESS-PROTOCOL.md`와 `docs/harness-protocol/`로 연결한다.
...
| 섹션 | 내용 |
| --- | --- |
| Current State | Phase, active plan, product backlog, harness backlog 포인터 |
| Active Work | ID, Priority, Status, Risk, Scope, Done Criteria, Verification |
| Checkpoints | 작업 단위 완료 조건과 검증 방법 |
...
하네스 운영 규칙의 활성 기준 문서. 상태 머신, 실행 gate, 실패 조건, 문서 lifecycle, recovery 기준을 요약하고 상세 규칙 파일로 연결한다. Agent가 따라야 하는 실행 프로토콜은 이 문서와 `docs/harness-protocol/`을 기준으로 판단한다.

**로드 조건:** workflow/harness 변경, command/rule 변경, 세션 프로토콜 점검 시.

### `docs/HARNESS-QUICK-REFERENCE.md`

세션 중 빠르게 확인하는 실행 카드. `/start`, `/work`, `/done` 흐름에서 필요한 최소 규칙, risk level, validation, naming 규칙을 담는다. 상세 판단이 필요하면 `docs/HARNESS-PROTOCOL.md`로 올라간다.

**로드 조건:** 세션 실행 규칙을 빠르게 확인할 때, 하네스 관련 작업 시작 시.

### `docs/harness-protocol/`

하네스 프로토콜의 카테고리별 상세 문서. session state machine, context loading, work item routing, document lifecycle, trigger/cascade, recovery/validation을 분리한다.

**로드 조건:** 해당 카테고리 규칙을 수정하거나 충돌 여부를 검토할 때. 전체 일괄 로드는 금지하고 필요한 파일만 선택한다.

### `docs/backlog/PHASE{n}.md`

다음 Phase에서 수행할 product/MSA template 후보 작업 목록. 우선순위(P0~P3), 선행 조건, Done Criteria, Verification을 포함한다. `/pick` 명령에서 product 작업을 고를 때의 입력 소스다.

### `docs/backlog/HARNESS.md`

AI workflow, command/rule, 문서 구조, hook/automation 후보를 관리하는 harness 전용 backlog. Product backlog와 분리해 Phase{n} 기능 계획이 하네스 개선 작업에 묻히지 않도록 한다.
...
| `java-spring.md` | services/, gateway/, common/ | `--profile spring-boot`에서만 포함되는 Java/Spring Boot 보조 규칙 |
...
현재 10개: `start`, `pick`, `register`, `work`, `resume`, `debug`, `doc`, `done`, `record-decision`, `health`
...
```mermaid
flowchart TD
    START([🟢 INIT\n새 세션 시작]) --> STARTUP["/start\nCLAUDE.md + docs/AGENT-WORKFLOW.md 자동 로드\nSTATUS.md 상단 섹션 확인"]
    ...
    RECOVER --> PLAN
    PASS -- 통과 --> CHECKPOINT["CHECKPOINT\nSTATUS.md 업데이트\n필요 시 DR/TODO/문서 cascade"]

    CHECKPOINT --> NEXT{추가 작업?}
    NEXT -- 예 --> ASSESS
    NEXT -- 아니오 --> DONE["/done\n세션 요약 생성"]
...
sequenceDiagram
    ...
    alt 검증 실패
        ...
        C->>F: CHECKPOINT - docs/STATUS.md 업데이트
        C->>F: 필요 시 DR/TODO/문서 cascade 반영
        C->>U: 완료 보고
    end
```

### 4-3. Context Load Priority

```mermaid
flowchart LR
    SESSION([세션 시작]) --> ENTRY{도구별 진입점}
    ENTRY -- Claude Code --> CLAUDE_AUTO["자동 로드\nCLAUDE.md\n→ docs/AGENT-WORKFLOW.md"]
    ENTRY -- Codex --> CODEX_AUTO["자동 참조\nAGENTS.md\n→ docs/AGENT-WORKFLOW.md"]
    ENTRY -- Cursor --> CURSOR_BOOT["session prompt\n+ .cursor/rules"]
    CLAUDE_AUTO --> CMD["/start 실행\n→ STATUS.md 읽기만 수행"]
    CODEX_AUTO --> CMD
    CURSOR_BOOT --> CMD
    ...
    ROUTE -- DR 영향 --> DECISIONS["docs/decisions/DR-*.md"]
    ...
    style CLAUDE_AUTO fill:#d4edda,stroke:#28a745
    style CODEX_AUTO fill:#d4edda,stroke:#28a745
    style STATUS fill:#d4edda,stroke:#28a745
    style HPROTO fill:#d4edda,stroke:#28a745
    style HDETAIL fill:#d4edda,stroke:#28a745
    style PLAN fill:#f8d7da,stroke:#dc3545
    style ARCHIVE fill:#f8d7da,stroke:#dc3545
    style TODO fill:#f8d7da,stroke:#dc3545
```

### 4-4. Context Load Condition Table
...
| --- | --- | --- | --- |
| 4 | `docs/HARNESS-QUICK-REFERENCE.md` | workflow/harness 작업 시작; 세션 실행 규칙 확인 | product 구현만 진행 |
...
| 9 | `docs/archive/*.md` | 이전 Phase 구현 맥락 복원; "Phase {n}에서 어떻게 했는지" 명시적 요청 | 현재 Phase 작업 (과거 이력 불필요) |
| 10 | `docs/PLAN.md` | PLAN-SUMMARY로 부족한 상세 근거; 아키텍처 변경 검토; Phase 계획 자체 수정; **신규 서비스·모듈 생성; Cross-service interaction 구현; Infra·배포 방식 변경; DB schema 변경** | 일반 구현·디버깅 (PLAN-SUMMARY로 충분) |
...
`scripts/create-harness.sh`로 하네스 파일 구조를 자동 생성한다. 기본값은 언어·프레임워크를 가정하지 않는 `generic` profile이며, 이 템플릿의 Java/Spring Boot 보조 규칙과 prompt가 필요할 때만 `--profile spring-boot`를 사용한다.
...
# Spring Boot/MSA 보조 규칙과 prompt까지 포함
...
# 기존 Spring Boot/MSA 프로젝트에 보조 규칙과 prompt까지 추가
...
| `.cursor/config.json`, `.cursor/rules/*.mdc` | 복사 | 기본 profile은 generic rules만 포함 |
...
- Spring Boot/MSA 전용 prompts (`02-scaffold-service`, `04-security-review`, `08-split-service`, `10-add-validation`, `11-add-resilience`, `12-performance-fix`, `13-add-metrics`, `14-write-migration`, `18-add-cache`, `21-create-layer`)
...
- [ ] **`CLAUDE.md`** (루트, 영어) 생성
  ...
  - `@docs/AGENT-WORKFLOW.md` import 라인 추가
...
- [ ] **`docs/harness-protocol/`** 상세 문서 생성
  - `01-session-state-machine.md`
  - `02-context-loading.md`
  - `03-work-items-and-naming.md`
  - `04-document-lifecycle.md`
  - `05-triggers-and-cascade.md`
  - `06-recovery-and-validation.md`
...
- [ ] **`.claude/commands/`** 파일 10개 복사 및 프로젝트 Phase명·backlog 경로 조정
  - `start.md`, `pick.md`, `register.md`, `work.md`, `resume.md`, `debug.md`, `doc.md`, `done.md`, `record-decision.md`, `health.md`
...
# Claude Code 시작 후
/start
# → CLAUDE.md, docs/AGENT-WORKFLOW.md 로드 확인
...
| T4 | TODO 분해 생성 | 2개 이상 조건 충족: 3+ 서브태스크, 3+ 파일, 2+ 서비스·모듈, L3, 다중 checkpoint, 세션 초과, 인계 가능성 / 사용자 명시 요청 | Claude 능동 제안 | STATUS.md Active Work Notes 또는 Scope |
...
| T7 | Harness protocol 업데이트 | docs/AGENT-WORKFLOW.md 워크플로우 규칙 변경 / `.claude/commands/*.md` 변경 / T1~T6 트리거 추가·변경 | Claude 능동 제안 | `HARNESS-PROTOCOL.md`, `docs/harness-protocol/`, 필요 시 `WORKFLOW-MANUAL.md` |
...
| 변경 원인 | 확인 대상 섹션 |
|-----------|----------------|
| 컨텍스트 로드 조건 변경 | `docs/harness-protocol/02-context-loading.md`, 필요 시 이 문서 §4-4 |
| 슬래시 커맨드 변경 | `docs/HARNESS-PROTOCOL.md`, `docs/HARNESS-QUICK-REFERENCE.md`, 필요 시 이 문서 §5 |
| DR 기준 변경 | `docs/harness-protocol/04-document-lifecycle.md`, 필요 시 이 문서 §6 |
| 트리거 추가·변경 | `docs/harness-protocol/05-triggers-and-cascade.md`, 필요 시 이 문서 §10 |
...
**cascade:**
- 산출물이 현재 상태 추적에 필요하면 STATUS Update Proposal 제안
- 산출물 작성 중 source 문서 오류를 발견하면 즉시 수정하지 않고 별도 작업 또는 STATUS Update Proposal로 분리
...
워크플로우 규칙 변경 / 커맨드 변경 / 트리거 변경
  └─ T7(Harness protocol 업데이트) → HARNESS-PROTOCOL.md / harness-protocol / 필요 시 WORKFLOW-MANUAL.md 동기화

비자명 이슈 해결 (환경 문제, 비직관적 원인)
  └─ T8(Troubleshooting 기록) → docs/troubleshooting/ 신규·업데이트 제안
     필요 시 docs/DEVELOPER-GUIDE.md 링크 추가
     (다른 트리거를 발동시키지 않음)

발표/보고 산출물 생성
  └─ T9(Artifact 생성) → docs/presentations/ 또는 docs/reports/ 신규·업데이트
     source 문서 변경은 별도 작업으로 분리
```
...
*이 문서는 사용자 매뉴얼이다. Claude instruction은 `CLAUDE.md`와 `docs/AGENT-WORKFLOW.md`, 하네스 실행 프로토콜은 `docs/HARNESS-PROTOCOL.md`와 `docs/harness-protocol/`을 따른다.*
