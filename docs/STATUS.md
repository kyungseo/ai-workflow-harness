...
Last updated: 2026-07-13 (축② provisional 해제 — agent-first upgrade 방향 확정)
...
- **첫 착수 추천:** **Manifest contract 정비** (W6, P2 — tolerant parser·source-ref·`generated_at`) — version-skew 3/3 실측으로 근거가 확정됐고, upgrade 절차 diet의 선행 권장 조건이다. (ai-deck replay는 CHORE-20260713-002로 완료 — 축② 해제됨)
...
- **W6 P2 후속:** upgrade 절차 diet(agent-first 경로 정식화, **canary gate** — 비오염 1건/fresh-session canary 전 default 전환 금지), UF-06 auto-merge default → thin-adapter화(순서 고정), UF-08 scaffold deny quick-fix, UF-01 product CI seam. P3: DR-034 manifest-target 분기 amend.
