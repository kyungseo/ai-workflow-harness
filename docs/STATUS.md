...
0. **P0 즉시** — 버전 체계 정의 + VERSION 정렬: semver 정책(major/minor/patch 기준) 문서화, 현재 `VERSION`(0.2.0) 값 재검토 및 조정, `VERSION` → `create-harness.sh` → `manifest.json` scaffold 경로 정합성 확인. 릴리즈 전 선행 필요.
1. **P0 즉시** — `VERIFICATION-COMMANDS.md` pointer 연결: `docs/AGENT-WORKFLOW.md` Verification Defaults + `skills/workflow/repo-health.md` + `docs/HARNESS-QUICK-REFERENCE.md` pointer 추가, scaffold 포함 여부 확정. (파일 생성·백로그 등록 완료 — 2026-06-08)
...
4. **Archive 대기** — Done Work 24개 누적. 다음 `/session-start` 시 일괄 처리 권장.
