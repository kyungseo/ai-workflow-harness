...
| --- | --- | --- | --- |
| L1 product surface | 간단 plan 승인. Quick Mode 가능 | Work checkpoint/discovery는 승인 불필요, 실행 후 보고 | validation, diff summary, commit message 보고 후 승인 |
...
- product 작업은 Quick Mode로 빠르게 닫고, workflow 변경은 L2 cascade로 다룬다.
...
- 단순 product 작업에 `/health --cascade`를 끌고 오지 않는다.
...
- Quick Mode를 product surface L1에만 제한
