...
## 10. Scaffold Product Repo Hook Policy

`tools/git-hooks/pre-commit`은 `ai-workflow-harness` source repo 전용이며, scaffold된 product repo에는 기본 포함되지 않는다.

| 대상 | Hook 정책 |
| --- | --- |
| source repo | 자동 설치 (`bash tools/git-hooks/install.sh`). branch isolation warning + shell syntax check |
| scaffold product repo | **기본 미포함.** `create-harness.sh`가 `tools/git-hooks/` 를 복사하지 않는다 |

**이유:**
- product repo의 lint/test/pre-commit stack과 충돌 위험
- protected files 목록(STATUS.md, AGENTS.md 등)이 harness source 전용 개념이므로 product repo에 직접 적용 부적합
- product repo branch naming이 Gitflow와 다를 수 있음

**product repo에 pre-commit hook이 필요한 경우:**
`tools/git-hooks/pre-commit`을 참고해 project-specific protected paths와 branch policy에 맞게 수정한 후 설치한다. harness hook을 그대로 복사하지 않는다.
