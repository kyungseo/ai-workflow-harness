...
| Workflow constants | `docs/AGENT-WORKFLOW.md` | framework convention(`Active state file`)과 framework Verification Defaults만. product runtime/build/architecture·product 검증 명령은 위 `Implementation Baseline`(PLAN-SUMMARY)에 둔다 (DR-043) |
...
- product runtime/build/architecture와 product 검증 명령은 `docs/PLAN-SUMMARY.md`의 `Implementation Baseline`/`Verification Defaults`에 둔다 (DR-043). `docs/AGENT-WORKFLOW.md`에는 framework convention만.
