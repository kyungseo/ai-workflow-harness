# BLOCK 3 — 도메인 모델 확정 + 01-schema.sql / 02-data.sql
...
## 3-2. 01-schema.sql 작성 (PostgreSQL 방언)

파일 위치: `infra/docker/init-sql/01-schema.sql`
...
파일 위치: `infra/docker/init-sql/02-data.sql`
...
- [x] 파일 상단 주석: `-- local/dev 전용 초기 데이터 (stg/prd 미포함)`

- [x] 테스트 계정 삽입 — **5개 계정**, BCrypt strength 12 해시값으로 직접 작성 (평문 절대 금지)
...
- [x] 샘플 Todo 삽입 — **`user_id` 하드코딩 금지, subselect 패턴 사용**
...
- [x] `.env` 파일 생성 (`.env.example` 기반으로 실제값 기입, HikariCP 환경변수 포함)
- [x] `docker compose -f infra/docker/docker-compose.yml up postgres redis -d` 기동
- [x] init-sql 자동 실행 확인 (컨테이너 로그에서 01-schema.sql, 02-data.sql 실행 확인)
- [x] 스키마 확인:
  ```bash
  docker exec -it <postgres_container> psql -U $DB_USERNAME -d $DB_NAME -c "\dt"
  ```
- [x] 초기 데이터 확인 — 사용자 5명, Todo 계정별 건수 검증:
  ```bash
  ...
  ```
- [x] `application-local.yml` SQL 로깅 활성화 확인 (`org.mybatis: DEBUG`)
- [x] ✅ **CP-1 통과 확인 후 BLOCK 4~6 진행**

---

## 완료 조건

- [x] PostgreSQL 기동 후 `users`, `todos` 테이블 생성 확인
- [x] 인덱스 3개 생성 확인
- [x] 5개 계정 데이터 확인 (`admin`, `user`, `user2`, `user3`, `user4`)
- [x] Todo 건수 확인: admin 3건, user 8건, user2 2건, user3/user4 0건
- [x] completed=true 항목 확인: admin 1건, user 3건
