...
- [ ] `.agents/skills/source-command-{name}/` → `.agents/skills/harness-{name}/` 디렉토리 rename (git mv × 11)
- [ ] 각 `SKILL.md` frontmatter `name`, 제목, "migrated source command" 문구를 `harness-{name}` 기준으로 수정
- [ ] `AGENTS.md` skill path 참조를 `.agents/skills/harness-{name}/SKILL.md`로 수정
- [ ] `docs/HARNESS-PROTOCOL.md` Cascade Matrix `source-command-*` → `harness-*` 수정
- [ ] `.claude/commands/health.md` 및 `.agents/skills/harness-health/SKILL.md` 내부 `source-command-*` 참조 수정
- [ ] cascade 문서 정렬 확인 및 suffix mapping 규칙 추가 (README.md, WORKFLOW-MANUAL.md, HARNESS-QUICK-REFERENCE.md, PLAN-SUMMARY.md, prompts/README.md, prompts/codex-session-start.md)
- [ ] `.claude/commands/{name}.md` ↔ `.agents/skills/harness-{name}/SKILL.md` suffix mapping 규칙을 health/cascade 문서에 명시
- [ ] `scripts/create-harness.sh` 검증 (`bash -n` 통과)
- [ ] scaffold 실제 생성 검증: `rg -n "source-command-" /private/tmp/hrn-028-*` 결과 없음
- [ ] `git diff --check` 통과
- [ ] `rg -n "source-command-" . --include="*.md" --include="*.sh" --include="*.json"` 결과 없음
...
- [ ] `.agents/skills/` rename (git mv × 11) + SKILL.md 내부 수정
- [ ] `AGENTS.md` + `HARNESS-PROTOCOL.md` + `health` 참조 수정
- [ ] cascade 문서 정렬 + suffix mapping 규칙 추가
- [ ] 검증 통과 + 전체 커밋
