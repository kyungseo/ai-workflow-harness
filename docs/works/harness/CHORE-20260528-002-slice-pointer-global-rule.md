...
- [ ] `docs/BEHAVIOR-PRINCIPLES.md` §2에 검토형 원칙 bullet이 추가됨 (전역 원칙, 약한 표현)
- [ ] `.cursor/rules/behavior-principles.mdc` §2에 대응 bullet이 추가됨 (English Only mirror)
- [ ] `docs/AGENT-WORKFLOW.md`에 slice & pointer 판단 기준이 추가됨
- [ ] `docs/HARNESS-PROTOCOL.md §16` Operating Principles에 bullet이 추가됨
- [ ] `.claude/rules/docs-workflow.md` MUST에 rule이 추가됨 (English Only)
- [ ] `.cursor/rules/workflow.mdc`에 Document Structure 섹션이 추가됨 (English Only)
- [ ] 6개 surface 판단 기준이 적용 강도에 맞게 일관됨 (전역 원칙: 검토형, workflow/protocol surface: 명확형)
- [ ] cascade 영향 문서 점검 완료
- [ ] STATUS Active pointer 및 Work index Active row 반영됨
