...
| Utility / Analysis | `/work-register`, `/work-debug`, `/work-brief`, `/cross-review`, `/record-decision`, `/repo-health` | 항목 등록, 문제 분석, brief/분류 정리, 선택적 review relay, 결정 기록, 상태 점검 |
| Utility / Optional | `/work-doc` (optional — `--with-optional`) | 발표/보고 산출물 생성 (scaffold default 미포함) |

`/cross-review` 사용법은 `docs/user/CROSS-REVIEW-MANUAL.md`를 참고한다. 단순 review/red-team 언급만으로 자동 기동하지 않고, 명시적 `/cross-review` 호출 또는 cross-agent review relay 요청이 필요하다.
