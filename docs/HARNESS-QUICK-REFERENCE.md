...
- **`/close`** = Work Done 처리 전용. 세션 종료 없이 Work 완료 처리(Done Criteria 확인 → status/actual_end 기입 → README Active→Done → STATUS pointer 제거 제안 → 선택적 archive). commit/PR이 이어지면 commit 전략(branch/push/PR 상태 기반 3-state 안내)을 먼저 제시하고 별도 commit gate에서 STATUS/Tracking Finalization을 보고한다. 실행 후 세션 계속.
...
새 작업 항목이 생기면 `/register`로 등록한다. 등록 시 TYPE(FEAT/PATCH/HOTFIX/CHORE)을 판단한다.

| 긴급도 / 성격 | 등록 위치 |
| --- | --- |
| 지금 바로 착수 (긴급 패치 등) | `docs/STATUS.md` Active Work → TYPE 판단 후 Work ID 제안 → `/work`로 연결 |
...
backlog 후보에는 Work ID를 선점하지 않는다. 제목/slug만 유지하고, Work ID는 `/work` 착수 승인 후 확정한다.
