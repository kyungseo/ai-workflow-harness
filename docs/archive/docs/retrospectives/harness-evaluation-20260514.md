...
- `docs/AGENT-WORKFLOW.md`를 자동 로드용 slim contract로 줄였다. 이는 Claude Code에서는 체감이 크다.
...
- `docs/AGENT-WORKFLOW.md`: 자동 로드 최소 규칙
...
- Claude Code는 루트 `CLAUDE.md`와 `docs/AGENT-WORKFLOW.md`를 자동 로드한다.
...
- `docs/AGENT-WORKFLOW.md`에 작업 유형별 routing decision table을 더 선명하게 추가한다.
