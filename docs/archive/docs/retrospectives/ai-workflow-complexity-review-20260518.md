...
Work 파일 변경은 내용에 따라 분리된다: checkpoint/discovery 추가는 L1(실행 후 보고), status: Done·actual_end·Done Criteria 전체 체크 등 Work Done 처리는 L2(제안→승인→실행). STATUS.md 변경(구 Layer 2)은 L2로 매핑된다.
commit 전 승인은 risk level과 무관하게 항상 별도 행으로 남긴다 — commit은 history를 남기는 행위라서 L1이어도 diff 확인 후 진행한다.

### S5 — `docs/WORKFLOW-MANUAL.md` 역할 명시적 재정의 [되돌리기 Low / 기대 효과 Medium → 실질 Medium]
...
### S1 — `docs/harness-protocol/` 6개 파일 → HARNESS-PROTOCOL.md 단일 통합 [되돌리기: 문서 구조 Low / cascade 작업량 Medium / 기대 효과 High]
...
HRN-019에서 드러났듯 `/health --cascade`가 복잡한 판단 엔진으로 커지면 그 자체가 또 하나의 무거운 규칙 체계가 된다. health.md 자체가 247줄 규모이며, 그 안의 cascade 섹션이 AI에게 "판단"을 요청하는 구조다.
