> ARCHIVED snapshot of `README.md` before CHORE-20260606-005. Historical only — live front-door is the repository root `README.md`.
...
1. 이 README에서 전체 구조와 기본 flow를 이해한다.
2. `scripts/create-harness.sh`로 target repo에 harness를 생성한다.
3. target repo에서 `/session-start` 또는 같은 intent로 첫 세션을 시작한다.
4. `docs/BOOTSTRAP.md`를 채워 Product Definition과 Implementation Baseline을 정리한다.
5. `/work-select` -> `/work-plan` -> 승인 후 실행 -> `/work-close` -> `/session-summary` 흐름으로 작업한다.
...
처음 보는 사용자는 이 구조를 먼저 이해하면 된다.

1. **Entry points**가 AI 도구별 시작점을 제공한다.
2. **Canonical workflow**가 실제 절차의 SSoT다.
3. **Adapters**가 Claude Code, Codex, Cursor의 실행 방식에 맞춰 canonical 절차를 호출한다.
4. **State files**가 현재 작업과 결정 근거를 repo 안에 남긴다.
5. **Scaffold**가 이 구조를 다른 target repo에 설치한다.

세션 시작 시 기본값으로 읽는 것은 [`docs/BEHAVIOR-PRINCIPLES.md`](docs/BEHAVIOR-PRINCIPLES.md), [`docs/AGENT-WORKFLOW.md`](docs/AGENT-WORKFLOW.md), [`docs/STATUS.md`](docs/STATUS.md) current sections뿐이다. 조건이 생길 때만 해당 문서를 추가 로드한다.
...
User-facing docs([`docs/WORKFLOW-MANUAL.md`](docs/WORKFLOW-MANUAL.md), [`docs/SCAFFOLD-ONBOARDING-GUIDE.md`](docs/SCAFFOLD-ONBOARDING-GUIDE.md))는 사용자가 흐름을 이해하기 위한 문서다. 평시 AI 실행 규칙의 권위는 canonical workflow와 shared operating rules에 두고, user-facing docs는 필요할 때만 확인한다.
...
Approval Matrix는 실행 전 승인, 상태 변경 승인, commit 전 승인을 하나의 기준으로 묶는다. 상세 표와 예외는 [`docs/AGENT-WORKFLOW.md`](docs/AGENT-WORKFLOW.md)의 Approval Matrix가 SSoT다.

README에서 기억할 것은 네 가지다.

| Gate | AI가 해야 할 일 |
| --- | --- |
| Scope | 실행 전에 Scope, Files, Verification, Risk, Reversal Cost를 보고한다. |
| State Change | `docs/STATUS.md`, Work Done, archive, Recent Decisions 같은 상태 변경은 대상 Work ID와 이유를 밝히고 승인받는다. |
| Validation | 검증 실패 상태에서는 checkpoint나 commit을 만들지 않는다. |
| Commit / PR | validation 결과, diff summary, 제안 commit message를 보고하고 별도 승인을 받는다. |

Quick Mode는 Product track의 작고 명확한 L1 작업에 한정한다. Harness/workflow surface(`entrypoint/workflow/protocol/command/rule/prompt/scaffold/status`)를 건드리면 기본 L2로 다룬다.

AI가 반드시 멈춰야 하는 경우: 승인된 scope 밖 파일을 고쳐야 할 때 / STATUS Active Work 항목을 추가·제거해야 할 때 / Work를 Done 처리해야 할 때 / commit하려 할 때 / validation 실패 후 복구 방향을 바꿔야 할 때.
...
사용자는 아래 command 이름으로 workflow에 진입한다. 상세 절차는 `skills/workflow/{name}.md`가 canonical SSoT이고, Claude/Codex/Cursor 표면은 이 절차를 호출하는 adapter다.

Claude Code는 slash command로 실행하고, Codex는 같은 intent를 repo-local skill routing으로 수행한다.
...
| [skills/workflow/](skills/workflow/) | workflow command별 canonical 절차 |
...
├── skills/
│   └── workflow/                          # command별 canonical workflow 절차
