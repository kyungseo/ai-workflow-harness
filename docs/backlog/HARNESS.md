...
| — | P2 | Candidate | L2 | Template document-set (D-21) source/product 경계 결정 |
...
#### Template document-set (D-21) source/product 경계 결정

**Cluster:** W2. Adopter Transition

**Task:** planning-pack D-21(14 core 문서 + decision/change + pack 조건부, `TEMPLATE-ACCEPTANCE` keystone)을 실제 product evidence 위에서 어떤 doc-set surface가 source-template-owned이고 어떤 게 product-owned인지 결정한다. full 문서셋 작성은 product 작업이고, 이 후보는 **source/product 경계와 흡수 관계만** 정한다.

**Evidence payload — document-set delta (CHORE-20260620-001, R1 F5):** `spring-modular-template` 5+ 사이클 산출물 대비 `document-set-scaffold.md` delta:
- `ADOPTER-RENAME` = product-owned seed(product가 compose/datasource/prometheus surface로 확장). source acceptance 승격 여부 미결.
- `TEMPLATE-ACCEPTANCE`(keystone) = pack acceptance evidence(run/test/reset/observe/remove)를 흡수해야 함.
- pack docs = `pack/{name}/README`(local-deploy, observability-export). 위치(`docs/packs/*` vs pack README)가 source template에서 미결.
- DR set = DR-030~033은 product 적용 기록(product-owned), source DR과 분리.

**Dependencies:** D-21, `document-set-scaffold.md`, `template-acceptance-matrix.md`, CHORE-20260620-001 evidence review, DR-032/033 pack docs precedent.

**Done Criteria:** 각 doc-set surface의 owner(source-template / scaffold-seed / product-owned)가 분리되고, `TEMPLATE-ACCEPTANCE`가 pack acceptance를 어떻게 흡수하는지, pack docs 표준 위치가 결정된다. full 문서 작성은 별도(product) 작업으로 둔다.

**Verification:** evidence delta 대조, scaffold doc-set 파일 영향 review, product doc-set과 source template 중복/누락 점검. Surface: scaffold · adopter cascade · README/GUIDE/MANUAL.
