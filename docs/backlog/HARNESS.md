...
| --- | --- | --- | --- | --- | --- | --- | --- |
| HRN-001 | P1 | Candidate | L2 | Stop hook 추가 — `/done` 없이 세션 종료 시 reminder 출력 | `settings.json` Stop hooks | 세션 종료 시 `/done` reminder가 출력됨 | 세션 종료 시 reminder 메시지 확인 |
| HRN-002 | P1 | Candidate | L2 | Hard enforcement 강화 — git hook + PostToolUse 실실행 | Manual protocol 안정화 | git pre-commit hook이 STATUS.md 최근 수정 여부를 체크; PostToolUse hook이 Java 파일 변경 시 `./gradlew checkstyleMain` 실제 실행 | hook 트리거 확인 및 lint 결과 확인 |
...
| HRN-008 | P2 | Candidate | L1 | Codex 전용 instruction 구조 검토 — `AGENTS.md` 도입 여부 결정 | Codex 사용 빈도 확인 | Codex가 자동으로 읽을 repo-level instruction을 둘지 결정하고, 도입 시 `AGENTS.md`, prompt/README, ignore/permission 정책을 정렬 | Codex 세션 시작 dry-run, 문서 링크 확인, `.gitignore`/`.claudeignore`/`.cursorignore`/`.claude/settings.json`과 충돌 여부 확인 |
| HRN-009 | P2 | Candidate | L1 | `docs/` 파일·디렉토리 naming audit — DR-008 기준 재검토 | `docs/decisions/DR-008-docs-filename-standard.md` | `docs/` 루트, `backlog/`, `decisions/`, `TODO/`, `harness-protocol/`, `archive/`, `retrospectives/`의 대소문자·hyphen 규칙을 재정의하거나 DR-008 예외를 명시; `VSCode-DevContainer구조.png`, legacy `TODO-BLOCK*.md`, archive snapshot 파일명 처리 방침 결정 | `find docs -maxdepth 3`, `rg` 참조 점검, case-only rename 필요 시 git-safe 절차 확인 |
