...
| --- | --- | --- | --- | --- | --- | --- | --- |
| — | P1 | Candidate | L3 | Gate enforcement runtime & environment (slug: `gate-enforcement-runtime-and-env`) — harness gate를 실제 런타임/스캐폴드 환경에서 동작시키는 메커니즘 통합 항목. **흡수: 구 HRN-002(hard enforcement hook) + 구 HRN-FUT-001(`.harness/config.json` SSOT).** 하위 범위: (a) commit-gate hard-stop/sentinel + validation-gap 감지 hook(DR-025 정책 구현, no-git/bootstrap/no-remote N/A-aware), (b) project-configurable 리스트(protected paths, bundling 대상 = T15/T16 산출물, check 로직)용 `.harness/config.json` SSOT — hook/rule 하드코딩 제거, (c) hook의 target scaffold 배포·install·CI 대안·product-adaptive 로직, (d) source-gitflow 환경 부트스트랩(git init→main→develop→origin→branch-protection per DR-020) 또는 명시적 N/A + remote 미준비 시 추적 후속 checkbox, production 성격과 onboarding 안내 연결(직교 유지). 자잘한 분산 대신 단일 맥락으로 유지하되 **단일 PR 아님 — (a)~(d)는 각각 별도 slice로 착수**(범위 묶음일 뿐 한 번에 끝내는 항목 아님) | DR-025(commit-gate 정책, 진행 중), DR-024(taxonomy), DR-020(branch ruleset). 흡수: 구 HRN-002·HRN-FUT-001. 범위 분리: `기존 source hook hygiene` 후보(현행 hook 점검). 관련 doc: `scaffold-target-maintenance-note`(framework/project 경계 user-facing 안내) | 하위 (a)~(d)별 착수 단위를 slice로 분해, hook hard-stop/override 동작·config 기반 리스트 적용·source-gitflow 환경 준비를 fresh scaffold에서 검증 | hook 트리거(bundling/validation-gap) 동작, `--check`/config 정합, source-gitflow target에서 develop/origin 준비 또는 N/A 안내 확인 |
...
| — | P2 | Candidate | L2 | 기존 source hook hygiene + commit-msg relation 점검 — **현행** `tools/git-hooks/pre-commit`(whitespace+branch isolation) 정확성 검토, commit-msg hook(build type) 관계 점검, 설치 절차 정비. **새 gate enforcement(bundling/validation hard-stop·config·scaffold 배포)는 범위 밖** — 그건 `gate-enforcement-runtime-and-env`가 SSoT다. hard block 전환(HRN-039 OQ-2) 결정은 그 통합 후보와 정합 유지 | 없음 (단 새 enforcement는 `gate-enforcement-runtime-and-env` 참조) | 현행 hook 검토 완료, 설치 확인(`ls .git/hooks/pre-commit`), GIT-WORKFLOW.md §6 설치 안내 현행화, 새 enforcement와 범위 중복 없음 | 현행 hook 설치 후 protect 파일 staged 시 경고/branch isolation 동작 확인 |
