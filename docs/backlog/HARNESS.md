...
| — | P2 | Candidate | L2 | WORKFLOW-MANUAL deep rewrite — README front-door 개편 이후 사용자용 deep reference를 canonical pointer 중심으로 재구조화 | CHORE-20260606-002 | Approval Matrix/trigger/command 절차 재서술을 줄이고 사용자 흐름·예시·WHY 중심으로 재정리, optional pack boundary 유지 | README/Quick/Manual command table consistency, old-name grep, source/target boundary read-through |
