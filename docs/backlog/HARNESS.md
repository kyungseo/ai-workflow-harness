...
| HRN-036 | P1 | In Progress | L2 | Public Release Clean Baseline Gate — main release PR 전 public clean 상태 점검과 release-ready snapshot 조건 명문화 | HRN-035 이후 public baseline 운영 | main PR 전 `STATUS.md` Active/Blockers/Next Actions, Work archive pending, README/onboarding/scaffold 경로, `/start` clean idle 시뮬레이션을 포함한 release checklist를 정의하고 develop→main release gate에 반영 | 문서 기준 release 시뮬레이션, `/start` clean idle 시뮬레이션, `git diff --check` |
