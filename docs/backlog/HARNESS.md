...
| — | P1 | Candidate | L2 | Decision workflow product coverage + DR lifecycle 정리 — `/repo-decision` 이름/설명이 product decision을 놓치게 만들 수 있는지 점검하고, product/harness 양쪽의 DR-worthy decision을 모두 포착하도록 절차/문구를 보강한다. 오래된 DR 관리는 Accepted/Accepted(Amended)/Superseded/partial supersede/parent-child/linked DR 구분을 명확히 하고, Superseded archive 이동과 최신 DR 연결 방식을 정리한다. Project decision 누락은 product repo에서 심각한 결함이 될 수 있으므로 P1로 다룬다. | `skills/workflow/repo-decision.md`, `docs/decisions/DECISION-TEMPLATE.md`, `docs/decisions/README.md`, DR-011/013/014 | product decision도 DR-worthy criteria로 자연스럽게 등록되고, 오래된 DR의 개정/대체/연계/삭제 기준이 혼동 없이 문서화됨 | product decision 예시 시뮬레이션, DR index 상태값 grep, Superseded/Amended/Linked DR sample 검토 |
