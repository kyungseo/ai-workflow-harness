...
| HRN-020 | P3 | Candidate | L1 | Command taxonomy 정리 — slash command를 session lifecycle / work lifecycle / utility·analysis 범주로 분류하고 문서 표기 명확화 | HRN-019 Done 이후 | command rename 없이 `/work`·`/resume`·`/close`는 Work lifecycle, `/start`·`/pick`·`/done`은 Session lifecycle, `/debug`·`/doc`·`/health`·`/record-decision`·`/register`는 Utility/analysis로 분류한다. `WORKFLOW-MANUAL.md`와 Quick Reference에 "무엇을 끝내는 명령인가"를 명확히 반영한다 | 커맨드 목록에서 범주 구분이 명시되고 rename 없이 의미 충돌 없음 확인 |
...
| HRN-022 | P2 | Candidate | L1 | Review-sensitive Work Done Criteria 정책 검토 — 사용자 최종 리뷰를 모든 Work가 아닌 필요한 Work의 Done Criteria로 선택 적용 | HRN-021-S4 사용자 리뷰 조건 실사용 결과 | 전역 강제 대신 `/close`가 Work 파일 Done Criteria에 포함된 사용자 최종 리뷰 조건을 확인하도록 할지 검토한다. 기본 포함 후보는 harness/workflow/user-facing manual/rule/command 변경, 정책·운영 절차 변경, 사용자가 명시한 "검토 후 Done" Work다. Quick Mode, 단순 오타·링크·기계적 정합성 패치, 테스트·검증으로 닫히는 구현 작업은 기본 제외로 둔다 | `docs/AGENT-WORKFLOW.md`, `.claude/commands/close.md`, `docs/HARNESS-PROTOCOL.md` 영향도 검토; 필요 시 DR-worthy 여부 판단 |
