...
| HRN-033 | P2 | Candidate | L2 | Scaffold onboarding git repository bootstrap — 신규 scaffold 직후 git 미초기화 상태 대응 | HRN-032와 병행 가능 | 신규/기존 scaffold에서 git repository 존재 여부를 onboarding 단계에서 확인하고, 미초기화 시 commit/PR/branch workflow를 Not Applicable로 처리하며 사용자 승인 기반 `git init`/initial commit 안내를 문서와 scaffold output에 반영 | git repo 없음/있음 시나리오별 `/start`·bootstrap 시뮬레이션, generated `README.md`/`BOOTSTRAP.md`/`STATUS.md` 안내 확인, scaffold 완료 메시지 검증 |
