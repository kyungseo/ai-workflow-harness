...
| — | P2 | Candidate | L2 | Deterministic source-parity 검사 CI/pre-commit 배선 검토 (DR-036 부분 재검토) |
...
#### Deterministic source-parity 검사 CI/pre-commit 배선 검토 (DR-036 부분 재검토)

**Cluster:** Enforcement & Lifecycle

**Task:** `check-default-template-parity.sh`(`.cursor/rules/workflow.mdc`·`.claude/rules/git-workflow.md` ↔ `scripts/templates/default/**`)와 `check-surface-mirror-parity.sh`(canonical↔claude↔agents mirror·prompt) 2종을 pre-commit 또는 CI(`ci.yml`)에 배선해 drift window를 닫는다. DR-036은 *tier2 runner 전체* 배선을 기각했으나, 이 2종은 **deterministic·빠름·source-only(adopter SKIP)·false-positive≈0**이라 DR-036이 우려한 "과중/adopter 오경보"에 거의 해당하지 않으므로 별개 검토 대상이다. 결론은 (a) 2종 배선 적용 **또는** (b) 무배선 유지가 낫다는 근거를 DR로 기록, 중 택1. **연계:** Deferred Ideas의 "Command/skill mirror atomicity 강화 — drift 자동 감지(CI/hook)" 항목(surface-mirror 각도)을 흡수/정합.

**Dependencies:** `docs/decisions/DR-036-f2-runner-wiring-decision.md`(무배선 결정), `tools/git-hooks/pre-commit`, `.github/workflows/ci.yml`, `scripts/tests/run-harness-checks.sh`.

**Trigger (실측):** FEAT-20260618-001에서 source `.cursor/rules/workflow.mdc`의 `work-brief` routing row가 template default에 미동기된 drift가 #196(2026-06-15)~release 직전까지 약 2.5주 무탐지. release gate full sweep(`run-harness-checks --all`)이 안전망으로 작동해 main 전에 잡았으나, 도입~탐지 사이 window가 실재함을 확인.

**Done Criteria:** 2종 parity 검사가 commit 또는 CI 시점에 자동 실행되어 template/mirror drift가 도입 시점에 차단되거나, 재검토 결과 무배선 유지가 더 낫다는 결론과 근거가 DR로 기록된다. adopter repo에서 source 부재 시 SKIP(N/A)이 정상 동작함을 확인한다.

**Verification:** 의도적 drift 주입 시 pre-commit/CI가 차단하는지, adopter(소스 부재) repo에서 SKIP 정상인지 확인. Surface: tool surface(hook/CI) · scaffold.
...
| — | Command/skill mirror atomicity 강화 — Work CP 단위 atomicity rule은 `docs/HARNESS-PARALLEL-WORK-CONTROLS.md`와 health command/skill에 반영됨(CHORE-20260528-001). drift 자동 감지(CI/hook)가 필요해지면 L3 Work로 등록 (→ surface-mirror 배선 각도는 backlog candidate "Deterministic source-parity 검사 CI/pre-commit 배선 검토"에서 함께 다룸) | command/skill mirror drift가 실제 운영 버그로 이어질 때 |
