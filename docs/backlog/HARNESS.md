...
| — | P1 | Candidate | L3 | Harness upgrade/migration 메커니즘 (`--upgrade`/`--refresh` 또는 update guide) — **driver: 공개 adopter `ai-deck-compiler`가 이 harness를 적용 중이며 upstream 변경 반영(업그레이드/마이그레이션)이 필요.** `--existing`은 신규 overlay용일 뿐 갱신 기능 아님. 이미 있는 `--check`(manifest sha256 drift 감지, CHORE-20260605-006) 위에 구축. 결정 사항: overwrite vs merge 정책, 사용자 커스터마이징/로컬 변경 보존(backup), version marker(`VERSION`), drift detection→apply 경로, release timing/criteria. **흡수: 구 HRN-FUT-008 + 구 Deferred "Scaffold template drift window 관리"(release timing). drift window 개념 자체는 `docs/HARNESS-PROTOCOL.md` §T12에 유지.** | `--check`/manifest(CHORE-20260605-006 Done), `VERSION` marker, DR-021 source/target boundary; gate 잔여작업(c2-B/c3/c4/d)과 독립 | adopter(`ai-deck-compiler` 또는 temp scaffold)를 과거 버전→현재 template로 업그레이드하는 경로(자동 또는 문서) 확립 + framework 파일 갱신과 사용자 커스터마이징 보존 양립 + dry-run/backup | 과거 scaffold→현재 template 업그레이드 시뮬레이션에서 framework 파일 갱신·사용자 변경 보존 동시 확인, upgrade 후 `--check` drift가 0이 되는지 |
| — | P2 | Candidate | L2 | 등록·착수 시점 정합성 hardening — (1) work 착수 시 backlog candidate row를 Candidate→Active로 상태 동기화(work-plan §3 보강; 현재는 ID만 갱신). (2) 착수 작업이 기존 backlog/Deferred Ideas 유사·동일 항목과 중복인지 확인 → 제거 또는 살릴 부분만 reframe. (3) 신규 등록(work-register/repo-decision/backlog/Deferred) 시 기존 미실행 항목과 유사·동일 점검 후 통폐합 — 통폐합 대상의 참조처를 grep으로 사전 확인한 뒤 제거/reframe. (4) index pairing 정합: retrospective 추가 시 `docs/retrospectives/README.md` 동반 갱신(works/decisions README 패턴과 통일), cascade/repo-health 점검 포함. **구현 원칙: command/skill에 경량 prompt-level 체크로만 추가(무거운 체크리스트 금지). 유사도 자동감지·참조그래프 자동화는 별도 L3 deferred.** | work-register/work-plan/repo-decision skill+command, `.claude/rules/docs-workflow.md`; (4)는 "외부화 실패모드 ① 라우팅 누락" 후보와 연계 | (1)~(4)가 해당 command/skill에 경량 절차로 반영되고 canonical→adapter cascade 정합 | work 착수 시 candidate 상태 전이 확인, 중복 등록 시도 시 통폐합 제안 동작, retrospective 추가 시 README 갱신 누락 감지 |
...
| — | Finalization gate의 standalone tracking-only commit 면제 — 번들할 substance가 없는 순수 tracking 정리(`docs/backlog/*`·`docs/STATUS.md`·`docs/works/*` 단독)는 DR-025 finalization-split hard-stop 대상이 아닌데 현 taxonomy로는 걸린다. gate를 tracking-only commit에 한해 면제 또는 advisory로 보정. 근거: 회고 `docs/retrospectives/harness-workflow-strictness-20260606.md` §5, slice (a) R1-N2(CHORE-20260606-006) | finalization gate 보정(c4 또는 DR-025 후속)을 착수할 때 |
