...
| — | Work ID collision 자동화 — Tracking Finalization에는 수동 확인 기준을 추가 완료. 병렬 feature에서 실제 collision이 반복되면 `/health --cascade` 또는 helper script로 `docs/works/**` 중복 Work ID 검사를 자동화 | collision이 실제 발생하거나 병렬 Active Work가 3개 이상 반복될 때 |
