...
| — | P1 | Candidate | L2 | Upgrade baseline source-ref policy and `--check` ref visibility |
...
#### Upgrade baseline source-ref policy and `--check` ref visibility

**Cluster:** W2. Adopter Transition

**Task:** adopter upgrade/apply evidence의 기본 baseline을 released `main` 또는 release tag로 정하고, develop/current checkout 기준 probe는 명시 예외 라벨을 필수화한다. Playbook probe checklist에 source ref 기록을 추가하고, `scripts/create-harness.sh --check`가 비교한 source ref(`git describe`/HEAD sha)를 출력하도록 하는 tooling gap을 별도 검토한다. DR-034 amendment 또는 신규 소형 DR 필요 여부를 판단한다.
