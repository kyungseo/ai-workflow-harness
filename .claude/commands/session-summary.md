...
이 파일은 Claude Code slash command adapter다. 상세 절차의 SSoT는 `skills/workflow/session-summary.md`다.
...
Claude Code에서 `/session-summary` slash command로 호출된다. 전달된 인자는 canonical 절차의 `$ARGUMENTS`로 해석한다.
