...
| **Output Path** | 최종본: `docs/presentations/` (PPTX) 또는 `docs/reports/` (문서). 작업 파일: `outputs/` 하위 경로. |
...
| --- | --- |
| Presentation, Deck (최종 PPTX) | `docs/presentations/` — 최종본만 보관 |
| Report, Review package, Decision brief, Markdown summary (최종본) | `docs/reports/` |
| 렌더링·검증용 작업 파일, 초안, 중간 산출물 | `outputs/presentations/` 또는 `outputs/reports/` |
...
- [ ] **Output Paths:** 최종본은 `docs/presentations/` 또는 `docs/reports/`에, 작업 파일은 `outputs/` 하위에 분리되었는가?
