...
- If `docs/BOOTSTRAP.md` exists and the repository is newly scaffolded or bootstrap tasks remain open, read it and propose project identity, Product backlog, Harness adjustments, and example-pack cleanup before ordinary work selection.
...
| --- | --- |
| `/start` | Read `docs/STATUS.md` current sections; if scaffold bootstrap is pending, include `docs/BOOTSTRAP.md`; summarize current state, bootstrap needs, candidates, needed context, risks |
| `/pick` | If Product backlog is empty or bootstrap remains open, use `docs/BOOTSTRAP.md` first; otherwise route to `docs/backlog/PHASE{n}.md` or `docs/backlog/HARNESS.md`, compare candidates, recommend one |
...
| Scaffold bootstrap / identity setup | `docs/BOOTSTRAP.md` → split Product backlog and Harness backlog proposals |
