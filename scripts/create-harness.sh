...
이 프로젝트는 Claude Code / Codex / Antigravity / Cursor 공통 AI 워크플로우 하네스를 포함합니다.
...
**Antigravity (Gemini 기반):** project root를 workspace로 열면 root \`AGENTS.md\`가 자동 로드되고 \`.agents/skills/\`(Codex surface)를 그대로 소비한다. 세션 첫 요청은 \`/session-start\` intent로 시작한다. 별도 진입 파일은 없으며, fallback은 \`prompts/codex-session-start.md\`를 공유한다.
...
- [ ] 권장 기본값: 신규 standalone(\`--existing\` overlay 아님 + 상위 git repo 없음)이면 \`git init\`을 기본 권장으로 제안하고 onboarding 산출물을 initial commit으로 남긴다. \`--existing\` overlay이거나 상위 repo 내부면 기존 VCS 정책을 먼저 확인하고 보수(아니오)를 기본으로 둔다
...
- [ ] \`docs/PLAN.md\` \`## 목표\` 작성 — 코드 여부와 무관하게 모든 프로젝트가 채운다 (roadmap/milestone은 optional)

## 3. Project Initialization

프로젝트 성격에 따라 둘 중 하나를 수행한다. 어느 쪽도 건너뛰지 않는다(no-code도 N/A 처리만 하고 넘어가지 않는다).
...
4. \`docs/PLAN.md\` \`## 목표\` — 프로젝트 목표/roadmap (코드 여부 무관, 모든 프로젝트)
5. \`docs/PLAN-SUMMARY.md\` Implementation Baseline — Runtime/Framework/Build/package 결정 (코드 프로젝트)
...
| 배포 또는 공개 방식 | — |
| 제품 핵심 workflow | — |
| AI 작업 도구 | Claude Code / Codex / Antigravity / Cursor |
| 주요 제약 조건 | — |
...
echo "  # Antigravity: project root를 workspace로 열면 AGENTS.md 자동 로드 후 /session-start intent로 시작"
