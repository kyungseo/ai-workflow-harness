...
Prompt library는 세 층으로 나눈다.

- **Session start fallback prompts**: command/adapter를 쓸 수 없을 때 세션을 시작하는 core fallback.
- **Optional generic task prompts**: 특정 framework를 전제하지 않는 portable task template. source repo에는 live example로 보관하고, scaffold target에는 `--with-optional`일 때만 포함한다.
- **Profile/stack example packs**: 특정 stack에 맞춘 prompt 예시. public repo에서는 확장 방식의 sample로 유지한다.
...
## 2) Optional Generic Task Prompts

이 prompt들은 AI Workflow Harness core가 아니다.
특정 framework를 전제하지 않는 reusable task example로 유지한다.
`scripts/create-harness.sh --with-optional`을 사용할 때 scaffold target에 포함된다.
...
- session-start fallback, optional generic task prompt, profile/stack example pack을 섞지 않는다.
- Optional generic task prompt와 profile/stack example pack은 현재 repository의 core runtime이 아니라 reusable sample로 설명한다.
