...
Canonical workflow procedure for `/work-register`.

이 파일은 workflow 상세 절차의 SSoT다. Tool-specific surface는 아래 adapter만 가진다.

| Tool | Adapter |
| --- | --- |
| Claude Code | `.claude/commands/work-register.md` |
| Codex | `.agents/skills/workflow-work-register/SKILL.md` |
| Antigravity | Codex adapter 재사용: `.agents/skills/workflow-work-register/SKILL.md` |
...
| Product 작업 | `docs/backlog/PRODUCT.md` |
...
- **Verification**: 검증 방법. 변경이 건드리는 surface를 명시한다 — tool surface · adopter cascade · canonical · scaffold · README/GUIDE/MANUAL 중 해당 항목

backlog 후보에는 Work ID를 선점하지 않는다. Work ID는 `/work-plan`로 착수 승인 후 Work 파일 생성 시 `<TYPE>-<YYYYMMDD>-<NNN>` 형식으로 확정한다. 형식 상세: `docs/HARNESS-NAMING-RULES.md`.

## Backlog Entry Format

`docs/backlog/HARNESS.md`와 `docs/backlog/PRODUCT.md`는 **2단 구조**를 사용한다.

**1단 — Summary 표에 행 추가** (스캔용):

```markdown
| — | P{n} | Candidate | L{n} | {Title 한 줄 요약} |
```

**2단 — Details 섹션에 블록 추가** (읽기용):

```markdown
#### {Title}

**Task:** {상세 설명. 연계 항목 있으면 끝에 **연계: ...** 추가}
...
두 위치에 동시에 추가한다. Summary 표 행만 추가하거나 Details 블록만 추가하면 불완전한 상태가 된다.
...
2. 등록 완료 후 `/work-plan [ID]`로 바로 이어갈지 물어봐.
