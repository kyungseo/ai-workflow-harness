...
**5b. Forward-Relevant Decision Triage (lightweight soft prompt)**

Done 처리 중(archive 결정 전), 해당 Work의 Discovery/Scope/비목표를 기준으로 **다음 세션에서 다시 볼 가치가 있는 forward-relevant 결정**이 남았는지 짧게 점검한다.

- 기본 대상은 `extraction-trigger`, `deferred`, `non-goal` 중 **forward-relevant token 또는 근거가 명시된 결정**이다.
- `follow-up` 일반 문구만 있는 항목은 기본 대상이 아니다.
- "그냥 안 하기로 한 비목표"는 승격하지 않는다. **근거는 충분하지만 지금은 미루는 결정**만 다룬다.
- 이 단계는 **soft prompt**다. Done 처리/commit/PR의 hard-stop이 아니다.

forward-relevant 결정이 있다고 판단되면, backlog row를 자동 생성하지 말고 Work 파일 `Discovery`에 아래 형식으로 **한 줄 메모만** 남긴다.

```text
- Needs-Triage: {topic} — {왜 다음 세션에서 다시 볼 가치가 있는지}
```

- 이 메모는 triage-only reminder다. 정식 backlog candidate 승격은 다음 `/session-start`, `/work-select`, `/work-plan`에서 owner 승인 하에 따로 판단한다.
- 같은 Work에 여러 건이 있더라도 1~3건의 짧은 메모로 제한한다. 이 단계가 별도 분류 체계나 heavy checklist로 커지지 않게 한다.
- archive step이 나중으로 미뤄져도 메모는 Work 파일에 남아 `/session-start`에서 다시 표면화된다.
...
**Next Actions Review (report-required, update-optional):**

- `docs/STATUS.md` Next Actions가 비었거나 추상적이면, 다음 세션에서 바로 착수 가능한 concrete action 1개를 제안한다.
- concrete action은 가능하면 `/work-plan` 또는 `/work-select`로 이어질 수 있는 단일 후보여야 한다.
- `docs/STATUS.md` 수정은 Approval Matrix state rules에 따라 사용자 승인 후에만 수행한다. 자동으로 STATUS를 수정하지 않는다.
- Next Actions 갱신이 불필요하다고 판단하면, 그 이유를 Completion Report에 남긴다.
...
0. `Discovery`에 `Needs-Triage:` 메모가 있으면, archive 이동 전에 그 메모를 한 번 다시 보여주고 "지금 triage할지 / 메모를 남긴 채 archive할지"를 묻는다. 이때도 backlog row를 자동 생성하지 않는다.
...
- Archive: {지금 처리 / 보류}
- Needs-Triage: {없음 / Discovery에 기록 / 승격 보류}
- STATUS.md: Active Work {ID} pointer 제거 {승인 대기 / 완료}
- Next Actions: {갱신 완료 / 갱신 제안 승인 대기 / 갱신 불필요 + 이유}
